...
export default function RadWorklistPage() {
  ...
  return (
    <div>
      <h1>Liste imagerie</h1>
      <p>Ordres d&apos;imagerie à traiter.</p>
      {loading && queue.length === 0 && pendingLocal.length === 0 ? (
        ...
        <div style={{ marginTop: 24, padding: 16, backgroundColor: "white", borderRadius: 4 }}>
          <p>Aucun ordre imagerie dans la liste.</p>
        </div>
      ) : (
                      ...
                      <td style={{ padding: 12 }}>
                        {worklistItemIsTerminal(item.status) ? (
                          ...
                          <>
                            ...
                            <Link href={`/app/rad-worklist/commande/${order.id}?ligne=${item.id}`} style={{ marginLeft: 4, fontSize: 13 }}>
                              {ui.common.view}
                            </Link>
                          </>
                        )}
