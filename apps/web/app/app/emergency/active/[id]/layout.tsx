...
const ED_SECTION_BY_CLINIC_SECTION: Record<ClinicCareAmbulatoryWorkspaceSection, ErWorkspaceSection> = {
  intake: "triage",
  "medical-evaluation": "providerMse",
  orders: "orders",
  medications: "mar",
  results: "results",
...
export default function EmergencyActiveEncounterLayout({ children }: { children: React.ReactNode }) {
  ...
  const navigate = useCallback(
    (section: ClinicCareAmbulatoryWorkspaceSection) => {
      const edSection = ED_SECTION_BY_CLINIC_SECTION[section] ?? "visitSummary";
      router.replace(emergencyActiveWorkspacePath(encounterId, { section: edSection }), { scroll: false });
    },
    [encounterId, router]
  );
  ...
  return (
    <div
      data-testid="emergency-with-ai-chart-review"
