...
import { en, es, fr } from "../../i18n/messages";
...
describe("D4SEC.1C.5C authorized single-admin staff UX",()=>{
 ...
 it("provides actionable English, French, and Spanish errors and direct-operation labels",()=>{for(const key of ["staff.add","staff.createEmployee","staff.created","staff.directNotice","error.RECENT_SESSION_MFA_REQUIRED","error.STAFF_PROVISION_FORBIDDEN","error.STAFF_ALREADY_PROVISIONED","error.STAFF_TARGET_INACTIVE"] as const){expect(en[key]).toBeTruthy();expect(fr[key]).toBeTruthy();expect(es[key]).toBeTruthy();expect(fr[key]).not.toBe(en[key]);expect(es[key]).not.toBe(en[key])}});
});
