...
describe("INP.1B.1 shared inpatient nursing navigation", () => {
  ...
  it("defines the exact shared chart header order", () => {
    const block = sections.slice(sections.indexOf("INPATIENT_SHARED_CHART_NAV_SECTIONS"), sections.indexOf("INPATIENT_PROVIDER_STICKY_NAV_SECTIONS"));
    const ids = [...block.matchAll(/\{ id: "([^"]+)"/g)].map((match) => match[1]);
    expect(ids).toEqual(["overview", "admission", "nursing", "orders", "medications", "results", "carePlan", "dischargePlanning", "timeline", "summary"]);
  });
  ...
  it("uses the shared list in the actual /chart runtime header", () => {
    ...
    expect(view).toContain('if (role === "CHART") return INPATIENT_SHARED_CHART_NAV_SECTIONS');
  ...
  it("resolves direct canonical and alias query sections on first render and refresh", () => {
    ...
    expect(view).toContain('parseInpatientWorkspaceSection(searchParams.get("section"))');
    expect(view).toContain('qs.set("section", next)');
  });
  ...
  it("provides explicit overview admission and assessment actions", () => {
    ...
    expect(overview).toContain("openAssessmentReassess");
    expect(overview).toContain("startAssessment");
  });
