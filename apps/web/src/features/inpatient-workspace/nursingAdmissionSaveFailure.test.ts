...
describe("MEDUI.INP.2B.2B nursing admission save failure classification", () => {
  ...
  it("classifies 401/403/404/5xx separately from network", () => {
    expect(
      classifyNursingAdmissionSaveFailure(Object.assign(new Error("auth"), { status: 401 })).kind
    ).toBe("AUTH");
    expect(
      classifyNursingAdmissionSaveFailure(Object.assign(new Error("forbidden"), { status: 403 })).kind
    ).toBe("FORBIDDEN");
    expect(
      classifyNursingAdmissionSaveFailure(Object.assign(new Error("missing"), { status: 404 })).kind
    ).toBe("NOT_FOUND");
    expect(
      classifyNursingAdmissionSaveFailure(Object.assign(new Error("down"), { status: 500 })).kind
    ).toBe("SERVER");
    expect(nursingAdmissionSaveFailureMessageKey("SERVER")).toBe("inpatientAdmissionInp2b2a.saveServer");
    expect(nursingAdmissionSaveFailureMessageKey("FORBIDDEN")).toBe(
      "inpatientAdmissionInp2b2a.saveForbidden"
    );
    expect(
      classifyNursingAdmissionSaveFailure(
