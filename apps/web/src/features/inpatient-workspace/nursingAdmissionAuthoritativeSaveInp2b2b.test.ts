...
describe("MEDUI.INP.2B.2B nursing admission authoritative-domain save hotfix", () => {
  it("keeps the serialized save coordinator for Confirm, Update, and section save", () => {
    ...
    expect(shell).toContain("historyEditor");
  ...
  it("mirrors new EN/FR save-failure keys without canonical backend codes", () => {
    ...
    expect(en.inpatientAdmissionInp2b2a.saveForbidden).toMatch(/authority/i);
    expect(fr.inpatientAdmissionInp2b2a.saveForbidden).toMatch(/autorité/i);
    expect(en.inpatientAdmissionInp2b2a.saveServer).not.toMatch(/connection/i);
  });
});
