...
const ROWS: readonly NursingBoardRow[] = [
  ...
  { id: "linesDrainsDevices", label: "Lines / drains / devices", group: "Devices and safety" },
  { id: "safetyPrecautions", label: "Safety precautions", group: "Devices and safety", options: choice("NONE", "FALL", "ASPIRATION", "SEIZURE", "BLEEDING", "OTHER") },
  { id: "nutritionHydration", label: "Nutrition / hydration", group: "Nutrition and output", options: choice("WNL", "TOLERATING", "POOR_INTAKE", "NPO", "HYDRATION_CONCERN") },
  { id: "ioMonitoring", label: "Intake / output status", group: "Nutrition and output", options: choice("ROUTINE", "STRICT", "NOT_REQUIRED", "CONCERN") },
  { id: "narrative", label: "Nursing narrative", group: "Narrative", kind: "textarea" },
] as const;
...
export function InpatientNursingAssessmentPanel({ encounterId, facilityId, patientId: _patientId, isLocked, onSaved }: { encounterId: string; facilityId: string; patientId: string; isLocked: boolean; onSaved: () => void | Promise<void> }) {
  ...
  const columns: NursingBoardColumn[] = useMemo(() => history.map((assessment) => ({ id: assessment.sessionId, occurredAt: assessment.authoredAt, status: assessment.status, author: assessment.authorDisplayName, values: toBoardValues(assessment) })), [history]);
  ...
  async function save() {
    ...
    setBusy(true);
    try {
      await apiFetch(`/encounters/${encodeURIComponent(encounterId)}/inpatient-nursing-assessments`, { method: "POST", facilityId, body: JSON.stringify({ ...draft, status: "SAVED" }) });
      setDraft(null); setCopied(new Set()); setMessage("Nursing assessment saved to immutable history.");
      await load(); await onSaved();
    } finally { setBusy(false); }
  }
  const latest = history.at(-1);
  const latestValues = latest ? toBoardValues(latest) : {};
  const french = language === "fr";
  return <div data-testid="inpatient-native-nursing-assessment">
    <NursingDocumentationBoard title={french ? "Évaluation infirmière" : "Nursing Assessment"} context={<>{french ? "Rencontre" : "Encounter"} {encounterId} · {french ? "La date, l’heure et l’identité de l’auteur sont attribuées par le serveur lors de l’enregistrement" : "Reassessment date, time and author identity are server-authored on save"}</>} rows={localizeRows(ROWS, french)} columns={columns} draft={draft ? toBoardValues(draft) : null} draftTime={draftTime} copiedFieldIds={copied} readOnly={isLocked} busy={busy} onChange={patch} onNew={() => begin(false)} onCopyPrevious={() => begin(true)} onSave={() => void save()} labels={french ? { clinicalFinding: "Constat clinique", noSaved: "Aucune évaluation enregistrée", addColumn: "+ Ajouter une colonne", copyPrevious: "Copier la précédente", save: "Enregistrer l’évaluation", notCharted: "Non documenté", currentSaved: "ACTUELLE · ENREGISTRÉE", saved: "ENREGISTRÉE", draft: "BROUILLON", summary: "Résumé infirmier" } : undefined} summary={<Summary values={latestValues} at={latest?.authoredAt} french={french} />} />
...
function Summary({ values, at, french }: { values: Record<string, NursingBoardValue>; at?: string; french: boolean }) {
  const row = (label: string, id: string) => <p style={{ margin: "7px 0" }}><strong>{label}:</strong> {String(values[id] ?? "Not charted").replaceAll("_", " ")}</p>;
  return <>{row(french ? "Moment / contexte" : "Timing / Context", at ? new Date(at).toLocaleString() : "")}{row(french ? "Douleur" : "Pain", "painScore")}{row(french ? "État mental" : "Mental status", "levelOfConsciousness")}{row(french ? "État respiratoire" : "Respiratory status", "respiratoryEffort")}{row(french ? "Mobilité / risque de chute" : "Mobility / fall risk", "fallRisk")}{row(french ? "Peau / plaies" : "Skin / wounds", "pressureInjuryConcern")}{row(french ? "Voies / dispositifs" : "Lines / devices", "linesDrainsDevices")}{row(french ? "Préoccupations de sécurité" : "Safety concerns", "safetyPrecautions")}</>;
}
