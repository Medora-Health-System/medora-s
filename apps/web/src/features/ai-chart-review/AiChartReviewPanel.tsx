...
const COPY: Record<"en" | "fr" | "es", Copy> = {
  en: {
    ...
    subtitle: "Deterministic clinical review",
  ...
  fr: {
    ...
    subtitle: "Révision clinique déterministe",
  ...
  es: {
    ...
    subtitle: "Revisión clínica determinística",
...
export function AiChartReviewPanel({
  ...
  const load = useCallback(async () => {
    ...
    setError(false);
    try {
      const raw = await apiFetch(`/ai/chart-review/${encodeURIComponent(encounterId)}`, { facilityId });
    ...
    >
      ...
      {!loading && error ? (
        <div role="alert" style={{ padding: 10, border: "1px solid #fecaca", borderRadius: 8, background: "#fef2f2" }}>
          ...
          <button type="button" onClick={() => void load()} style={{ marginTop: 8, border: "1px solid #fca5a5", borderRadius: 7, background: "#fff", padding: "5px 8px", fontSize: 11 }}>
            {copy.retry}
          </button>
        </div>
      ) : null}
          ...
          <article key={suggestion.id} style={{ borderTop: "1px solid #e2e8f0", padding: "12px 0" }}>
            ...
            {section ? (
              <button
                ...
                style={{ marginTop: 9, border: "1px solid #99f6e4", background: "#f0fdfa", color: "#115e59", borderRadius: 7, padding: "6px 9px", fontSize: 11.5, fontWeight: 600, cursor: "pointer" }}
              >
                {copy.reviewSection}
              </button>
            ) : null}
