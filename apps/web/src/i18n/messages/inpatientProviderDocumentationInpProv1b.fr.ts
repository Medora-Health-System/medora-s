/** INP.PROV.1B — Espace documentation médecin. */
export const inpatientProviderDocumentationInpProv1bFr = {
  title: "Documentation médecin",
  subtitle: "Créer, modifier et gérer les notes médicales de cette rencontre.",
  viewOnly: "Lecture seule — la rédaction clinique exige l’autorité PROVIDER.",
  noteType: "Type de note",
  noteTypes: {
    PROGRESS: "Note d’évolution",
    HP: "H&P",
  },
  signSave: "Signer et enregistrer",
  saveDraft: "Enregistrer le brouillon",
  signing: "Signature…",
  saving: "Enregistrement…",
  savedAt: "Enregistré {time}",
  saveFailedStatus: "Échec de l’enregistrement",
  characters: "Caractères : {n}",
  notes: "Notes",
  newNote: "+ Nouvelle note",
  showAllNotes: "Afficher toutes les notes ({n})",
  draft: "Brouillon",
  signed: "Signée",
  amended: "Amendée",
  corrected: "Corrigée",
  review: "Revue",
  tabs: {
    note: "Note",
    templates: "Modèles",
    smartPhrases: "SmartPhrases",
    flowsheets: "Feuilles de suivi",
    dictate: "Dicter",
  },
  toolbar: {
    attach: "Joindre",
    smartAssist: "Assistant",
    insert: "Insérer",
    orders: "Ordonnances",
    undo: "Annuler",
    redo: "Rétablir",
  },
  soap: {
    SUBJECTIVE: "Subjectif",
    OBJECTIVE: "Objectif",
    ASSESSMENT: "Évaluation",
    PLAN: "Plan",
  },
  dictateInto: {
    SUBJECTIVE: "Dicter dans Subjectif",
    OBJECTIVE: "Dicter dans Objectif",
    ASSESSMENT: "Dicter dans Évaluation",
    PLAN: "Dicter dans Plan",
    HP: "Dicter dans le H&P",
  },
  attachMenu: {
    labs: "Laboratoire",
    imaging: "Imagerie",
    results: "Résultats",
  },
  insertMenu: {
    phrase: "SmartPhrase",
    labRow: "Résultat sélectionné",
  },
  smartAssist: {
    title: "Assistant",
    beta: "BÊTA",
    suggestions: "Suggestions",
    review: "Revue",
    insert: "Insérer",
    preview: "Aperçu",
    contextOnly: "Contexte seulement",
    regenerate: "Actualiser",
    empty: "Aucune suggestion à partir des données du dossier.",
    reviewEmpty: "Aucun écart de complétude détecté.",
    reviewHpRosMissing: "ROS non documenté",
    reviewHpExamMissing: "Examen physique non documenté",
    reviewHpUnsigned: "H&P non signé",
    kinds: {
      lab: "Laboratoire récent",
      order: "Ordonnance existante",
      problem: "Problème actif",
      note: "Note signée récente",
    },
  },
  rightTabs: {
    smartAssist: "Assistant",
    patientContext: "Contexte patient",
  },
  patientContext: {
    title: "Contexte patient",
    allergies: "Allergies",
    codeStatus: "Statut de code",
    isolation: "Isolement",
    attending: "Médecin traitant",
    primaryDx: "Diagnostic principal",
    viewMore: "Voir plus",
    notDocumented: "—",
  },
  recentLabs: {
    title: "Laboratoire récent",
    viewAll: "Tout voir",
    empty: "Aucun résultat de laboratoire récent.",
    high: "Élevé",
    low: "Bas",
  },
  recentNotes: {
    title: "Notes récentes",
    viewAll: "Tout voir",
    empty: "Aucune note signée.",
  },
  encounterOrders: {
    title: "Ordonnances de la rencontre",
    count: "{n} ordonnances",
    addOrder: "+ Ajouter une ordonnance",
    empty: "Aucune ordonnance active pour cette rencontre.",
  },
  dictate: {
    currentTarget: "Cible de dictée actuelle : {section}",
    targetAction: "Placer le curseur dans {section}",
  },
  templates: {
    title: "Modèles",
    blankSoap: "SOAP vierge",
    standardDaily: "Note d’évolution quotidienne standard",
    applied: "Modèle appliqué.",
    overwriteConfirm: "Remplacer le contenu actuel de la note par ce modèle ?",
  },
  smartPhrases: {
    title: "SmartPhrases",
    subtitle: "Phrases cliniques rapides (pas une bibliothèque personnelle enregistrée).",
    appendHint: "Cliquez une phrase pour l’ajouter au Plan.",
  },
  flowsheets: {
    title: "Feuilles de suivi",
    empty: "Aucun signe vital ni bilan entrées/sorties pour cette rencontre.",
    vitalsTitle: "Tendances des signes vitaux (24 h)",
    measure: "Mesure",
    current: "Actuel",
    previous: "Précédent",
    trend: "Tendance 24 h",
    ioTitle: "Entrées / sorties (24 h)",
    intake24: "Entrées",
    output24: "Sorties",
    balance24: "Bilan",
    viewResults: "Voir les résultats",
  },
  previewTitle: "Aperçu",
  closePreview: "Fermer",
  noActiveNote: "Créez ou sélectionnez une note pour commencer.",
  signFailed: "Impossible de signer la note.",
  saveFailed: "Impossible d’enregistrer le brouillon.",
  loadFailed: "Impossible de charger la documentation médecin.",
  hpSystems: {
    allSystemsNegative: "Systèmes non documentés négatifs",
    normalExam: "Compléter les systèmes non documentés (normal)",
    replaceAllRos: "Remplacer le ROS (tout négatif)",
    replaceAllExam: "Remplacer l’examen (tout normal)",
    clear: "Effacer",
    replaceConfirm:
      "Ceci remplacera les constatations actuelles, y compris les exceptions. Continuer ?",
    additionalNotes: "Notes complémentaires",
    edit: "Modifier",
    negative: "Négatif",
    positive: "Positif",
    normal: "Normal",
    abnormal: "Anormal",
    notAssessed: "Non évalué",
    signedReadOnly: "Signé — lecture seule",
  },
};
