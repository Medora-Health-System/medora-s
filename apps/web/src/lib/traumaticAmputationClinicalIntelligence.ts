...
export function resolveAmputationContextFromDiagnosis(input: AmputationDiagnosisInput): AmputationContext {
 ...
 if(code.startsWith("S68")||/finger|thumb|doigt|pouce/.test(text)) regions=["finger_thumb"]; else if(code.startsWith("S48")||code.startsWith("S58")||/hand|upper extremity|main|bras/.test(text)) regions=["hand_upper_extremity"]; else if(code.startsWith("S98")||/toe|orteil/.test(text)) regions=["toe"]; else if(code.startsWith("S78")||code.startsWith("S88")||/foot|lower extremity|pied|jambe/.test(text)) regions=["foot_lower_extremity"]; else if(/amputation|traumatic amputation|amputation traumatique/.test(text)) regions=["unspecified"];
 ...
 const extent:AmputationExtent=/multiple|several|plusieurs/.test(text)?"multiple_digits":/partial|partielle/.test(text)?"partial":/complete|complete|total/.test(text)?"complete":"unknown";
 const dischargeFamilyId=!regions.length?null:extent==="partial"?"trauma_amputation_partial":extent==="complete"?"trauma_amputation_complete":regions.includes("finger_thumb")?"trauma_amputation_finger_thumb":regions.includes("hand_upper_extremity")?"trauma_amputation_hand_upper_extremity":regions.includes("toe")?"trauma_amputation_toe":regions.includes("foot_lower_extremity")?"trauma_amputation_foot_lower_extremity":"trauma_amputation_generic";
