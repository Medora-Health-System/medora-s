...
import { providerDocumentationDiarrheaComplaintIntelEn } from "@/i18n/messages/providerDocumentationDiarrheaComplaintIntel.en";

export const DIARRHEA_GOLD_STANDARD_BUNDLES = [
  ADULT_DIARRHEA_COMPLAINT_INTEL,
  PEDIATRIC_DIARRHEA_COMPLAINT_INTEL,
  DIARRHEA_COMPLAINT_V1_INTEL,
] as const;

export const DIARRHEA_COMPLAINT_INTEL_TEMPLATE_IDS = [
  "adult_diarrhea",
  "diarrhea",
...
function messagesForBundle(bundle: (typeof DIARRHEA_GOLD_STANDARD_BUNDLES)[number]) {
  ...
  const namespaceObject =
    providerDocumentationDiarrheaComplaintIntelEn[
      namespace as keyof typeof providerDocumentationDiarrheaComplaintIntelEn
    ] ?? {};
...
describe("providerDocumentationDiarrheaTrackC — MEDUI.ED.ME.2B-R", () => {
  it.each(DIARRHEA_GOLD_STANDARD_BUNDLES)("passes Track C compliance", (bundle) => {
    expect(collectTrackCViolations(bundle)).toEqual([]);
    expect(() => assertTrackCCompliance(bundle)).not.toThrow();
  });

  it.each(DIARRHEA_GOLD_STANDARD_BUNDLES)("has seven-section MDM gold standard", (bundle) => {
  ...
  it.each(DIARRHEA_GOLD_STANDARD_BUNDLES)("covers cannot-miss diagnoses", (bundle) => {
    const suffixes = (bundle.mdmDifferentialSynthesis ?? []).map(fragmentKeySuffix);
    expect(suffixes).toEqual(
      expect.arrayContaining([
        "diffCDifficileColitis",
        "diffToxicMegacolon",
        "diffIschemicColitis",
        "diffSepsis",
        "diffSevereDehydration",
        "diffHemolyticUremicSyndrome",
        "diffAppendicitis",
  ...
  it("covers pediatric wet diaper and caregiver chips", () => {
    const suffixes = (PEDIATRIC_DIARRHEA_COMPLAINT_INTEL.hpi ?? []).map(fragmentKeySuffix);
    expect(suffixes).toEqual(
      expect.arrayContaining([
        "hpiCaregiverPresent",
        "hpiParentReportsDiarrhea",
        "hpiStillMakingWetDiapers",
        "hpiDecreasedWetDiapers",
      ])
    );
  });

  it.each(DIARRHEA_GOLD_STANDARD_BUNDLES)("has no duplicate fragment keys", (bundle) => {
    const keys = flattenComplaintIntelligenceKeys(bundle);
    expect(new Set(keys).size).toBe(keys.length);
  });

  it.each(DIARRHEA_GOLD_STANDARD_BUNDLES)("has chart-ready EN i18n for every key", (bundle) => {
  ...
  it.each(DIARRHEA_GOLD_STANDARD_BUNDLES)("does not use prohibited HPI or exam key suffixes", (bundle) => {
  ...
  it.each(DIARRHEA_COMPLAINT_INTEL_TEMPLATE_IDS)("exposes MDM.1 workspace bindings for %s", (templateId) => {
