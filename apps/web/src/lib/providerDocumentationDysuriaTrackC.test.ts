...
import { DYSURIA_COMPLAINT_V1_INTEL, flattenComplaintIntelligenceKeys } from "./providerDocumentationComplaintIntelligence";
import {
...
export const DYSURIA_GOLD_STANDARD_BUNDLES = [DYSURIA_COMPLAINT_V1_INTEL] as const;

export const DYSURIA_COMPLAINT_INTEL_TEMPLATE_IDS = ["dysuria_complaint_v1"] as const;

function fragmentKeySuffix(fragmentKey: string): string {
  return fragmentKey.split(".").pop() ?? "";
}

function messagesForBundle(bundle: (typeof DYSURIA_GOLD_STANDARD_BUNDLES)[number]) {
  ...
  const namespaceObject =
    providerDocumentationDysuriaComplaintIntelEn[
      namespace as keyof typeof providerDocumentationDysuriaComplaintIntelEn
    ] ?? {};
...
describe("providerDocumentationDysuriaTrackC — MEDUI.ED.ME.2PD-R", () => {
  it.each(DYSURIA_GOLD_STANDARD_BUNDLES)("passes Track C compliance", (bundle) => {
    expect(collectTrackCViolations(bundle)).toEqual([]);
    expect(() => assertTrackCCompliance(bundle)).not.toThrow();
  });

  it.each(DYSURIA_GOLD_STANDARD_BUNDLES)("has seven-section MDM gold standard", (bundle) => {
  ...
  it.each(DYSURIA_GOLD_STANDARD_BUNDLES)("covers cannot-miss diagnoses", (bundle) => {
    ...
    expect(suffixes).toEqual(
      expect.arrayContaining([
        ...
        "diffPregnancyAssociatedInfection",
  ...
  it.each(DYSURIA_GOLD_STANDARD_BUNDLES)("has no duplicate fragment keys", (bundle) => {
    const keys = flattenComplaintIntelligenceKeys(bundle);
    expect(new Set(keys).size).toBe(keys.length);
  });

  it.each(DYSURIA_GOLD_STANDARD_BUNDLES)("has chart-ready EN i18n for every key", (bundle) => {
  ...
  it.each(DYSURIA_GOLD_STANDARD_BUNDLES)("does not use prohibited HPI or exam key suffixes", (bundle) => {
  ...
  it.each(DYSURIA_COMPLAINT_INTEL_TEMPLATE_IDS)("exposes MDM.1 workspace bindings for %s", (templateId) => {
