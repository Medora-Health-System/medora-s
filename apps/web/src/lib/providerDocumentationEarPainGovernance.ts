/**
 * MEDUI.ED.ME.2R — Ear pain / otitis sticky note governance.
 *
 * Discovery note: expected dedicated ear template IDs (otitis_media_complaint_v1,
 * otitis_externa_complaint_v1, etc.) are not yet in the catalog. Governed templates:
 * `ear_pain_otitis_complaint_v1` (adult) and `ear_pain` (pediatric legacy).
 */
...
export const EAR_PAIN_GOVERNED_TEMPLATE_IDS = [
  "ear_pain_otitis_complaint_v1",
  "ear_pain",
] as const satisfies readonly ProviderDocumentationTemplateId[];

export type EarPainGovernedTemplateId = (typeof EAR_PAIN_GOVERNED_TEMPLATE_IDS)[number];

const EAR_PAIN_GOVERNED_TEMPLATE_ID_SET = new Set<string>(EAR_PAIN_GOVERNED_TEMPLATE_IDS);

/** Fragment keys that must not appear when an ear pain / otitis template is active. */
export const EAR_PAIN_DENIED_STICKY_NOTE_FRAGMENT_KEYS = [
...
export const EAR_PAIN_DENIED_HPI_FRAGMENT_PREFIXES = [
...
export const EAR_PAIN_ALLOWED_EXAM_SECTION_IDS = [
  "general",
  "heent",
  "neuroPsych",
  "reassessment",
...
const DENIED_STICKY_NOTE_FRAGMENT_KEY_SET = new Set<string>(EAR_PAIN_DENIED_STICKY_NOTE_FRAGMENT_KEYS);

export function templateUsesEarPainStickyNoteGovernance(
  templateId: ProviderDocumentationTemplateId | null
): templateId is EarPainGovernedTemplateId {
  return Boolean(templateId && EAR_PAIN_GOVERNED_TEMPLATE_ID_SET.has(templateId));
}

export function isEarPainDeniedStickyNoteFragment(fragmentKey: string): boolean {
  if (DENIED_STICKY_NOTE_FRAGMENT_KEY_SET.has(fragmentKey)) return true;
  return EAR_PAIN_DENIED_HPI_FRAGMENT_PREFIXES.some((prefix) => fragmentKey.startsWith(prefix));
}
...
function filterChipGroups<T extends StickyNoteChipGroup<{ fragmentKey: string }>>(groups: T[]): T[] {
  return groups
    .map((group) => ({
      ...group,
      chips: group.chips.filter((chip) => !isEarPainDeniedStickyNoteFragment(chip.fragmentKey)),
    }))
    .filter((group) => group.chips.length > 0);
}

export function resolveEarPainRosChipGroupsForTemplate<T extends StickyNoteChipGroup<{ fragmentKey: string }>>(
  templateId: ProviderDocumentationTemplateId | null,
  baseGroups: T[]
): T[] {
  if (!templateUsesEarPainStickyNoteGovernance(templateId)) return baseGroups;
  return filterChipGroups(baseGroups);
}

export function resolveEarPainExamChipGroupsForTemplate<
  T extends StickyNoteExamChipGroup<{ fragmentKey: string }>,
>(templateId: ProviderDocumentationTemplateId | null, baseGroups: T[]): T[] {
  if (!templateUsesEarPainStickyNoteGovernance(templateId)) return baseGroups;
  return baseGroups
    .filter((group) => (EAR_PAIN_ALLOWED_EXAM_SECTION_IDS as readonly string[]).includes(group.sectionId))
    .map((group) => ({
      ...group,
      chips: group.chips.filter((chip) => !isEarPainDeniedStickyNoteFragment(chip.fragmentKey)),
    }))
...
export function resolveEarPainHpiChipGroupsForTemplate<T extends ProviderDocumentationHpiDimensionGroup>(
  templateId: ProviderDocumentationTemplateId | null,
  baseGroups: T[]
): T[] {
  const groups = resolveBaseHpiChipGroupsForTemplate(templateId, baseGroups);
  if (!templateUsesEarPainStickyNoteGovernance(templateId)) return groups;
  return groups
    .map((group) => ({
      ...group,
      chips: group.chips.filter((chip) => !isEarPainDeniedStickyNoteFragment(chip.fragmentKey)),
...
export function filterEarPainMdmTemplateOptionsForTemplate(
  templateId: ProviderDocumentationTemplateId | null,
  options: MdmTemplateOption[]
): MdmTemplateOption[] {
  if (!templateUsesEarPainStickyNoteGovernance(templateId)) return options;
  return options.filter((option) => !isEarPainDeniedStickyNoteFragment(option.fragmentKey));
}

export function collectEarPainVisibleStickyNoteFragmentKeys({
  templateId,
  rosBaseGroups,
  examBaseGroups,
  hpiBaseGroups,
}: {
  templateId: EarPainGovernedTemplateId;
  ...
  for (const group of resolveEarPainHpiChipGroupsForTemplate(templateId, hpiBaseGroups)) {
    for (const chip of group.chips) keys.add(chip.fragmentKey);
  }
  for (const group of getTemplateHpiDimensionGroups(templateId) ?? []) {
    for (const chip of group.chips) {
      if (!isEarPainDeniedStickyNoteFragment(chip.fragmentKey)) keys.add(chip.fragmentKey);
    }
  }

  for (const group of resolveEarPainRosChipGroupsForTemplate(templateId, rosBaseGroups)) {
    for (const chip of group.chips) keys.add(chip.fragmentKey);
  }
  for (const group of resolveEarPainExamChipGroupsForTemplate(templateId, examBaseGroups)) {
    for (const chip of group.chips) keys.add(chip.fragmentKey);
  }
  ...
  if (template) {
    for (const fieldKeys of Object.values(template.fields)) {
      for (const fragmentKey of fieldKeys ?? []) {
        if (!isEarPainDeniedStickyNoteFragment(fragmentKey)) keys.add(fragmentKey);
      }
    }
