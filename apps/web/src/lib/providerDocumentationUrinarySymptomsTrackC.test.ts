...
import { providerDocumentationUrinarySymptomsComplaintIntelEn } from "@/i18n/messages/providerDocumentationUrinarySymptomsComplaintIntel.en";

export const URINARY_SYMPTOMS_GOLD_STANDARD_BUNDLES = [UTI_URINARY_SYMPTOMS_COMPLAINT_INTEL] as const;

export const URINARY_SYMPTOMS_COMPLAINT_INTEL_TEMPLATE_IDS = ["urinary_symptoms"] as const;

function fragmentKeySuffix(fragmentKey: string): string {
  return fragmentKey.split(".").pop() ?? "";
}

function messagesForBundle(bundle: (typeof URINARY_SYMPTOMS_GOLD_STANDARD_BUNDLES)[number]) {
  ...
  const namespaceObject =
    providerDocumentationUrinarySymptomsComplaintIntelEn[
      namespace as keyof typeof providerDocumentationUrinarySymptomsComplaintIntelEn
    ] ?? {};
...
describe("providerDocumentationUrinarySymptomsTrackC — MEDUI.ED.ME.2A-R", () => {
  it.each(URINARY_SYMPTOMS_GOLD_STANDARD_BUNDLES)("passes Track C compliance", (bundle) => {
    expect(collectTrackCViolations(bundle)).toEqual([]);
    expect(() => assertTrackCCompliance(bundle)).not.toThrow();
  });

  it.each(URINARY_SYMPTOMS_GOLD_STANDARD_BUNDLES)("has seven-section MDM gold standard", (bundle) => {
  ...
  it.each(URINARY_SYMPTOMS_GOLD_STANDARD_BUNDLES)("covers cannot-miss diagnoses", (bundle) => {
    ...
    expect(suffixes).toEqual(
      expect.arrayContaining([
        ...
        "diffPregnancyAssociatedUti",
        "diffSepticStone",
        "diffUrinaryRetention",
        "diffRenalAbscess",
  ...
  it.each(URINARY_SYMPTOMS_GOLD_STANDARD_BUNDLES)("has no duplicate fragment keys", (bundle) => {
    const keys = flattenComplaintIntelligenceKeys(bundle);
    expect(new Set(keys).size).toBe(keys.length);
  });

  it.each(URINARY_SYMPTOMS_GOLD_STANDARD_BUNDLES)("has chart-ready EN i18n for every key", (bundle) => {
  ...
  it.each(URINARY_SYMPTOMS_GOLD_STANDARD_BUNDLES)("does not use prohibited HPI or exam key suffixes", (bundle) => {
  ...
  it.each(URINARY_SYMPTOMS_COMPLAINT_INTEL_TEMPLATE_IDS)("exposes MDM.1 workspace bindings for %s", (templateId) => {
