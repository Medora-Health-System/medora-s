...
describe("providerDocumentationModel", () => {
  ...
  it("persists optional provider title metadata for the Clinic signature summary", () => {
    const metadata = buildProviderDocumentationMetadata({ encounterMode: "AMBULATORY", savedAt: "2026-09-21T21:00:00.000Z", savedBy: "Dr Test", savedByTitle: "Physician" });
    const payload = buildProviderDocumentationSavePayload({ previousNursingAssessment: {}, state: { ...emptyProviderDocumentationWorkspaceState(), hpi: "Follow-up" }, metadata });
