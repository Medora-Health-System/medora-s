/**
 * MEDUI.ED.ME.2Q — Dental / oral pain sticky note governance.
 *
 * Discovery note: expected dedicated dental template IDs (dental_abscess_complaint_v1,
 * dry_socket_complaint_v1, etc.) are not yet in the catalog. `dental_pain_infection_complaint_v1`
 * is the sole dental/oral chief complaint template discovered.
 */
...
export const DENTAL_ORAL_GOVERNED_TEMPLATE_IDS = [
  "dental_pain_infection_complaint_v1",
] as const satisfies readonly ProviderDocumentationTemplateId[];

export type DentalOralGovernedTemplateId = (typeof DENTAL_ORAL_GOVERNED_TEMPLATE_IDS)[number];

const DENTAL_ORAL_GOVERNED_TEMPLATE_ID_SET = new Set<string>(DENTAL_ORAL_GOVERNED_TEMPLATE_IDS);

/** Fragment keys that must not appear when a dental/oral template is active. */
export const DENTAL_ORAL_DENIED_STICKY_NOTE_FRAGMENT_KEYS = [
...
export const DENTAL_ORAL_DENIED_HPI_FRAGMENT_PREFIXES = [
...
export const DENTAL_ORAL_ALLOWED_EXAM_SECTION_IDS = [
  "general",
  "heent",
  "skin",
  "reassessment",
...
const DENIED_STICKY_NOTE_FRAGMENT_KEY_SET = new Set<string>(DENTAL_ORAL_DENIED_STICKY_NOTE_FRAGMENT_KEYS);

export function templateUsesDentalOralStickyNoteGovernance(
  templateId: ProviderDocumentationTemplateId | null
): templateId is DentalOralGovernedTemplateId {
  return Boolean(templateId && DENTAL_ORAL_GOVERNED_TEMPLATE_ID_SET.has(templateId));
}

export function isDentalOralDeniedStickyNoteFragment(fragmentKey: string): boolean {
  if (DENIED_STICKY_NOTE_FRAGMENT_KEY_SET.has(fragmentKey)) return true;
  return DENTAL_ORAL_DENIED_HPI_FRAGMENT_PREFIXES.some((prefix) => fragmentKey.startsWith(prefix));
}
...
function filterChipGroups<T extends StickyNoteChipGroup<{ fragmentKey: string }>>(groups: T[]): T[] {
  return groups
    .map((group) => ({
      ...group,
      chips: group.chips.filter((chip) => !isDentalOralDeniedStickyNoteFragment(chip.fragmentKey)),
    }))
    .filter((group) => group.chips.length > 0);
}

export function resolveDentalOralRosChipGroupsForTemplate<T extends StickyNoteChipGroup<{ fragmentKey: string }>>(
  templateId: ProviderDocumentationTemplateId | null,
  baseGroups: T[]
): T[] {
  if (!templateUsesDentalOralStickyNoteGovernance(templateId)) return baseGroups;
  return filterChipGroups(baseGroups);
}

export function resolveDentalOralExamChipGroupsForTemplate<
  T extends StickyNoteExamChipGroup<{ fragmentKey: string }>,
>(templateId: ProviderDocumentationTemplateId | null, baseGroups: T[]): T[] {
  if (!templateUsesDentalOralStickyNoteGovernance(templateId)) return baseGroups;
  return baseGroups
    .filter((group) => (DENTAL_ORAL_ALLOWED_EXAM_SECTION_IDS as readonly string[]).includes(group.sectionId))
    .map((group) => ({
      ...group,
      chips: group.chips.filter((chip) => !isDentalOralDeniedStickyNoteFragment(chip.fragmentKey)),
    }))
...
export function resolveDentalOralHpiChipGroupsForTemplate<T extends ProviderDocumentationHpiDimensionGroup>(
  templateId: ProviderDocumentationTemplateId | null,
  baseGroups: T[]
): T[] {
  const groups = resolveBaseHpiChipGroupsForTemplate(templateId, baseGroups);
  if (!templateUsesDentalOralStickyNoteGovernance(templateId)) return groups;
  return groups
    .map((group) => ({
      ...group,
      chips: group.chips.filter((chip) => !isDentalOralDeniedStickyNoteFragment(chip.fragmentKey)),
...
export function filterDentalOralMdmTemplateOptionsForTemplate(
  templateId: ProviderDocumentationTemplateId | null,
  options: MdmTemplateOption[]
): MdmTemplateOption[] {
  if (!templateUsesDentalOralStickyNoteGovernance(templateId)) return options;
  return options.filter((option) => !isDentalOralDeniedStickyNoteFragment(option.fragmentKey));
}

export function collectDentalOralVisibleStickyNoteFragmentKeys({
  templateId,
  rosBaseGroups,
  examBaseGroups,
  hpiBaseGroups,
}: {
  templateId: DentalOralGovernedTemplateId;
  ...
  for (const group of resolveDentalOralHpiChipGroupsForTemplate(templateId, hpiBaseGroups)) {
    for (const chip of group.chips) keys.add(chip.fragmentKey);
  }
  for (const group of getTemplateHpiDimensionGroups(templateId) ?? []) {
    for (const chip of group.chips) {
      if (!isDentalOralDeniedStickyNoteFragment(chip.fragmentKey)) keys.add(chip.fragmentKey);
    }
  }

  for (const group of resolveDentalOralRosChipGroupsForTemplate(templateId, rosBaseGroups)) {
    for (const chip of group.chips) keys.add(chip.fragmentKey);
  }
  for (const group of resolveDentalOralExamChipGroupsForTemplate(templateId, examBaseGroups)) {
    for (const chip of group.chips) keys.add(chip.fragmentKey);
  }
  ...
  if (template) {
    for (const fieldKeys of Object.values(template.fields)) {
      for (const fragmentKey of fieldKeys ?? []) {
        if (!isDentalOralDeniedStickyNoteFragment(fragmentKey)) keys.add(fragmentKey);
      }
    }
