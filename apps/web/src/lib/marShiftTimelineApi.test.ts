...
describe("marShiftTimelineApi (M1.8B.7K.2)", () => {
  ...
  it("builds correct /facilities/:facilityId/mar-shift-timeline URL", async () => {
    ...
    // Encounter-scoped MAR must not send assignedToUserId (shared clinical access).
    expect(apiFetch).toHaveBeenCalledWith(
      "/facilities/fac-1/mar-shift-timeline?shiftCode=7A_7P&encounterId=enc-1&includeCompleted=true&includeUpcoming=false",
      { facilityId: "fac-1" }
    );
  });

  it("keeps assignedToUserId for facility-wide boards without encounterId", async () => {
    const { apiFetch } = await import("@/lib/apiClient");
    vi.mocked(apiFetch).mockResolvedValue({
      enabled: true,
      facility: { id: "fac-1", name: "St. Mary Hospital" },
    ...
    expect(apiFetch).toHaveBeenCalledWith(
      "/facilities/fac-1/mar-shift-timeline?shiftCode=7A_7P&assignedToUserId=nurse-1&includeCompleted=true&includeUpcoming=false",
      { facilityId: "fac-1" }
    );
  });
