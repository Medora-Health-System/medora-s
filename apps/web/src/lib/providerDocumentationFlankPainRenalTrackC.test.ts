...
import { flattenComplaintIntelligenceKeys, FLANK_PAIN_COMPLAINT_INTEL } from "./providerDocumentationComplaintIntelligence";
...
export const FLANK_PAIN_RENAL_GOLD_STANDARD_BUNDLES = [
  FLANK_PAIN_COMPLAINT_INTEL,
  FLANK_PAIN_RENAL_COMPLAINT_V1_INTEL,
  FLANK_PAIN_COMPLAINT_V1_INTEL,
] as const;

export const FLANK_PAIN_RENAL_COMPLAINT_INTEL_TEMPLATE_IDS = [
  "flank_pain",
...
function messagesForBundle(bundle: (typeof FLANK_PAIN_RENAL_GOLD_STANDARD_BUNDLES)[number]) {
  ...
  const namespaceObject =
    providerDocumentationFlankPainRenalComplaintIntelEn[
      namespace as keyof typeof providerDocumentationFlankPainRenalComplaintIntelEn
    ] ?? {};
...
describe("providerDocumentationFlankPainRenalTrackC — MEDUI.ED.ME.2PB-R", () => {
  it.each(FLANK_PAIN_RENAL_GOLD_STANDARD_BUNDLES)("passes Track C compliance", (bundle) => {
    expect(collectTrackCViolations(bundle)).toEqual([]);
    expect(() => assertTrackCCompliance(bundle)).not.toThrow();
  });

  it.each(FLANK_PAIN_RENAL_GOLD_STANDARD_BUNDLES)("has seven-section MDM gold standard", (bundle) => {
  ...
  it.each(FLANK_PAIN_RENAL_GOLD_STANDARD_BUNDLES)("covers cannot-miss diagnoses", (bundle) => {
  ...
  it.each(FLANK_PAIN_RENAL_GOLD_STANDARD_BUNDLES)("covers CVA tenderness and solitary kidney red flags", (bundle) => {
    const examSuffixes = Object.values(bundle.physicalExam ?? {})
      .flat()
      .map(fragmentKeySuffix);
    const rfSuffixes = (bundle.rosRedFlags ?? []).map(fragmentKeySuffix);
    expect(examSuffixes).toEqual(
      expect.arrayContaining(["examRightCvaTenderness", "examLeftCvaTenderness", "examNoCvaTenderness"])
    );
    expect(rfSuffixes).toEqual(expect.arrayContaining(["rfSolitaryKidney", "rfInfectedStoneConcern"]));
  });

  it.each(FLANK_PAIN_RENAL_GOLD_STANDARD_BUNDLES)("has no duplicate fragment keys", (bundle) => {
    const keys = flattenComplaintIntelligenceKeys(bundle);
    expect(new Set(keys).size).toBe(keys.length);
  });

  it.each(FLANK_PAIN_RENAL_GOLD_STANDARD_BUNDLES)("has chart-ready EN i18n for every key", (bundle) => {
  ...
  it.each(FLANK_PAIN_RENAL_GOLD_STANDARD_BUNDLES)("does not use prohibited HPI or exam key suffixes", (bundle) => {
  ...
  it.each(FLANK_PAIN_RENAL_COMPLAINT_INTEL_TEMPLATE_IDS)(
    "exposes MDM.1 workspace bindings for %s",
    (templateId) => {
