...
export function ClinicalWritingGuard({ language }: { language: SupportedLanguage }) {
  ...
  useEffect(() => {
    ...
    const clearForFocusChange = (event: Event) => { const focused = event.target; setActiveSuggestion((current) => (current?.element === focused ? current : null)); };
    ...
    return () => { observer.disconnect(); document.removeEventListener("input", updateSuggestion, true); document.removeEventListener("keyup", updateSuggestion, true); document.removeEventListener("focusin", clearForFocusChange, true); };
  }, [language]);
  ...
  return (
    <button type="button" data-testid={activeSuggestion.kind === "grammar" ? "clinical-writing-grammar-suggestion" : "clinical-writing-lexicon-suggestion"} aria-label={label} onMouseDown={(event) => event.preventDefault()} onClick={acceptSuggestion} style={{ position: "fixed", left: activeSuggestion.left, top: activeSuggestion.top, zIndex: 10000, maxWidth: 332, padding: "7px 10px", border: "1px solid #cbd5e1", borderRadius: 8, background: "#fff", color: "#0f172a", fontSize: 12, fontWeight: 600, boxShadow: "0 4px 14px rgba(15, 23, 42, 0.12)", cursor: "pointer" }}>
      {label}
    </button>
  );
}
