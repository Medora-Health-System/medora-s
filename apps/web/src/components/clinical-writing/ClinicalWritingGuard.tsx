...
type ActiveClinicalSuggestion = ClinicalWritingSuggestion & { element: HTMLTextAreaElement; left: number; top: number };
...
export function ClinicalWritingGuard({ language }: { language: SupportedLanguage }) {
  ...
  useEffect(() => {
    ...
    const observer = new MutationObserver((mutations) => {
      for (const mutation of mutations) for (const node of mutation.addedNodes) if (node instanceof Element) applyWritingSupport(node, language);
    });
    ...
    const updateSuggestion = (event: Event) => {
      ...
      setActiveSuggestion({ ...suggestion, element, left: Math.max(8, Math.min(rect.left, window.innerWidth - 300)), top: Math.min(rect.bottom + 6, window.innerHeight - 48) });
    };

    const clearForFocusChange = (event: Event) => {
      const focused = event.target;
      setActiveSuggestion((current) => (current?.element === focused ? current : null));
    };

    observer.observe(document.body, { childList: true, subtree: true });
    document.addEventListener("input", updateSuggestion, true);
    document.addEventListener("keyup", updateSuggestion, true);
    document.addEventListener("focusin", clearForFocusChange, true);
    return () => {
      observer.disconnect();
      document.removeEventListener("input", updateSuggestion, true);
      document.removeEventListener("keyup", updateSuggestion, true);
      document.removeEventListener("focusin", clearForFocusChange, true);
    };
  }, [language]);
  ...
  const acceptSuggestion = () => {
    ...
    if (!element.isConnected || document.activeElement !== element) { setActiveSuggestion(null); return; }
  ...
  return (
    <button type="button" data-testid="clinical-writing-lexicon-suggestion" aria-label={clinicalWritingSuggestionLabel(language, activeSuggestion.replacement)} onMouseDown={(event) => event.preventDefault()} onClick={acceptSuggestion} style={{ position: "fixed", left: activeSuggestion.left, top: activeSuggestion.top, zIndex: 10000, maxWidth: 292, padding: "7px 10px", border: "1px solid #cbd5e1", borderRadius: 8, background: "#fff", color: "#0f172a", fontSize: 12, fontWeight: 600, boxShadow: "0 4px 14px rgba(15, 23, 42, 0.12)", cursor: "pointer" }}>
