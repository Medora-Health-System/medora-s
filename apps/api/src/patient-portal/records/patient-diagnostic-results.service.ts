...
function sanitizeResultData(value: unknown): unknown { if(Array.isArray(value)) return value.map(sanitizeResultData); if(!value||typeof value!=="object") return value; const out:Record<string,unknown>={}; for(const [key,nested] of Object.entries(value as Record<string,unknown>)){ if(key==="attachments"||key==="dataBase64") continue; out[key]=sanitizeResultData(nested); } return out; }
