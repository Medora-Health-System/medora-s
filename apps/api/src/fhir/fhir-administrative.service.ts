...
export class FhirAdministrativeService {
  ...
  private async rows(type: string, facilityId: string, v: Record<string, string>, take: number, cursor?: string): Promise<Array<{ resourceType: string; id: string; [key: string]: unknown }>> {
    ...
    if (type === "Location") {
      const requestedOrganization = v.organization?.replace(/^Organization\//, "");
    ...
    if (type === "PractitionerRole") {
      const requestedOrganization = v.organization?.replace(/^Organization\//, "");
      const rows = await this.prisma.userRole.findMany({ where: { facilityId: requestedOrganization && requestedOrganization !== facilityId ? "__not_visible__" : facilityId, isActive: true, role: { code: { in: CLINICAL_ROLES } }, ...(v._id ? { id: v._id } : {}), ...(v.practitioner ? { userId: v.practitioner.replace(/^Practitioner\//, "") } : {}), ...(cursor ? { AND: { id: { gt: cursor } } } : {}) }, include: { role: true }, orderBy: { id: "asc" }, take });
