...
export class FhirMachineIdentityService {
  ...
  connectionInfo() {
    const configured = process.env.FHIR_PUBLIC_BASE_URL?.trim().replace(/\/$/, "");
    const fhirBaseUrl = configured || "https://api.medoras.com/fhir";
    return {
      fhirBaseUrl,
      tokenUrl: `${fhirBaseUrl}/auth/token`,
      metadataUrl: `${fhirBaseUrl}/metadata`,
      authMethod: "client_credentials",
      secretPolicy: "Client secrets are displayed once. Medora stores only an Argon2id hash.",
    };
  }

  async listClients(integrationId: string): Promise<IntegrationClientRow[]> {
    await this.requireIntegration(integrationId);
    return this.prisma.$queryRaw<IntegrationClientRow[]>(Prisma.sql`
  ...
  async provisionClient(
    adminUserId: string,
    integrationId: string,
    input: { facilityId: string; displayName?: string; sourceSystemIdentifier?: string; scopes?: string[]; credentialExpiresAt?: string },
  ) {
    ...
    const facilityGrant = integration.facilities.find((entry: any) => entry.facilityId === input.facilityId && entry.active !== false && !entry.revokedAt);
    ...
    if (!scopes.length || scopes.some((scope) => !allowed.has(scope))) throw new BadRequestException("Client scopes exceed integration permissions");

    const expiresAt = input.credentialExpiresAt ? new Date(input.credentialExpiresAt) : null;
    if (expiresAt && (Number.isNaN(expiresAt.valueOf()) || expiresAt <= new Date())) throw new BadRequestException("Credential expiry must be in the future");
    ...
    try {
      await this.prisma.$transaction(async (tx) => {
        ...
        await this.audit.log(AuditAction.UPDATE, "FHIR_INTEGRATION_CLIENT", {
          ...
          entityId: clientId,
          critical: true,
          metadata: { event: "FHIR_M2M_CLIENT_PROVISIONED", integrationId, keyId, scopeCount: scopes.length },
        });
      });
    } catch (error: any) {
      if (String(error?.message ?? "").includes("IntegrationClient_integrationId_facilityId_key")) {
        throw new ConflictException("A FHIR client already exists for this integration and facility");
      }
      throw error;
    }
  ...
  async rotateCredential(adminUserId: string, integrationId: string, clientId: string, input: { expiresAt?: string } = {}) {
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      `);
      await this.audit.log(AuditAction.UPDATE, "FHIR_INTEGRATION_CLIENT", { tx, userId: adminUserId, facilityId: client.facilityId, entityId: clientId, critical: true, metadata: { event: "FHIR_M2M_CREDENTIAL_ROTATED", integrationId, keyId } });
    });
    return { clientId, keyId, clientSecret, expiresAt, ...this.connectionInfo(), secretDisplayPolicy: "This new secret is shown once." };
  }
  ...
  async revokeClient(adminUserId: string, integrationId: string, clientId: string) {
    ...
    await this.prisma.$transaction(async (tx) => {
      await tx.$executeRaw(Prisma.sql`
        ...
        SET "active" = false, "revokedAt" = COALESCE("revokedAt", ${now}), "revokedById" = COALESCE("revokedById", ${adminUserId}), "updatedById" = ${adminUserId}, "updatedAt" = ${now}
        WHERE "id" = ${clientId} AND "integrationId" = ${integrationId}
      `);
      await tx.$executeRaw(Prisma.sql`
        UPDATE "interop"."IntegrationClientCredential"
        SET "revokedAt" = COALESCE("revokedAt", ${now}), "revokedById" = COALESCE("revokedById", ${adminUserId})
        WHERE "clientId" = ${clientId}
      `);
      await this.audit.log(AuditAction.UPDATE, "FHIR_INTEGRATION_CLIENT", { tx, userId: adminUserId, facilityId: client.facilityId, entityId: clientId, critical: true, metadata: { event: "FHIR_M2M_CLIENT_REVOKED", integrationId } });
    });
  ...
  async issueToken(input: { clientId: string; keyId: string; clientSecret: string; facilityId: string; scopes?: string[] }) {
    ...
    const [row] = await this.prisma.$queryRaw<Array<any>>(Prisma.sql`
      SELECT c."id", c."integrationId", c."facilityId", c."active", c."revokedAt",
    ...
    const accessToken = await this.jwt.signAsync({ tokenType: "fhir-m2m", clientId: row.id, integrationId: row.integrationId, facilityId: row.facilityId, scopes: requested, kid: row.credentialKeyId, jti }, {
      secret: this.signingSecret(),
      issuer: process.env.FHIR_M2M_ISSUER?.trim() || "medora-s",
      audience: process.env.FHIR_M2M_AUDIENCE?.trim() || "medora-fhir",
      subject: row.id,
      expiresIn,
    });
  ...
  async auditAccess(principal: FhirMachinePrincipal, resourceType: string, interaction: string, requestId?: string) {
    await this.audit.log(AuditAction.VIEW, "FHIR_INTEGRATION_ACCESS", { facilityId: principal.facilityId, entityId: principal.clientId, metadata: { integrationId: principal.integrationId, keyId: principal.keyId, resourceType, interaction, requestId: requestId || undefined } });
  }

  async testCredentials(input: { clientId: string; keyId: string; clientSecret: string; facilityId: string }) {
    const token = await this.issueToken({ ...input });
    return { ok: true, tokenIssued: true, expiresIn: token.expires_in, scopes: token.scope.split(" ").filter(Boolean), ...this.connectionInfo() };
  }
  ...
  private async consumeRateLimit(clientId: string, facilityId: string) {
    ...
    const [bucket] = await this.prisma.$queryRaw<Array<{ requestCount: number }>>(Prisma.sql`
      ...
      ON CONFLICT ("clientId", "facilityId", "windowStart") DO UPDATE SET "requestCount" = bucket."requestCount" + 1, "updatedAt" = CURRENT_TIMESTAMP
  ...
  private async requireClient(integrationId: string, clientId: string) {
    const [client] = await this.prisma.$queryRaw<Array<{ id: string; facilityId: string; active: boolean }>>(Prisma.sql`SELECT "id", "facilityId", "active" FROM "interop"."IntegrationClient" WHERE "id" = ${clientId} AND "integrationId" = ${integrationId} LIMIT 1`);
  ...
  private tokenTtlSeconds() {
    const parsed = Number(process.env.FHIR_M2M_TOKEN_TTL_SECONDS ?? 300);
