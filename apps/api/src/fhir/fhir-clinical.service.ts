...
export class FhirClinicalService {
  ...
  async find(type: ClinicalResourceType, facilityId: string, query: Record<string, unknown>) {
    ...
    if (type === "Condition") {
      if (v["verification-status"] && v["verification-status"] !== "confirmed") throw new BadRequestException("Unsupported verification-status");
      ...
      rows = await this.prisma.diagnosis.findMany({ where: { facilityId, ...(id ? { id } : {}), ...(patient ? { patientId: patient } : {}), ...(encounter ? { encounterId: encounter } : {}), ...(v.code ? { code: this.token(v.code) } : {}), ...(status ? { status: status as any } : {}), ...(v["recorded-date"] ? { createdAt: this.day(v["recorded-date"]) } : {}), ...(parsed.cursor ? { id: { gt: parsed.cursor } } : {}) }, orderBy: { id: "asc" }, take: parsed.count + 1 });
      ...
      rows = await this.prisma.result.findMany({ where: { facilityId, ...(id ? { id } : {}), ...(parsed.cursor ? { id: { gt: parsed.cursor } } : {}), ...(v.date ? { createdAt: this.day(v.date) } : {}), orderItem: { id: basedOn, catalogItemType: { in: TYPES }, order: { facilityId, ...(patient ? { patientId: patient } : {}), ...(encounter ? { encounterId: encounter } : {}) } } }, include: { orderItem: { include: { order: true } } }, orderBy: { id: "asc" }, take: parsed.count + 1 });
      if (v.status) rows = rows.filter((r) => this.reportStatus(r) === v.status);
    } else {
  ...
  private mapCondition(r: any) { return { resourceType: "Condition", id: r.id, clinicalStatus: { coding: [{ system: "http://terminology.hl7.org/CodeSystem/condition-clinical", code: r.status === "RESOLVED" ? "resolved" : "active" }] }, verificationStatus: { coding: [{ system: "http://terminology.hl7.org/CodeSystem/condition-ver-status", code: r.status === "REMOVED" ? "entered-in-error" : "confirmed" }] }, code: r.codeSource === "ICD10_CATALOG" ? { coding: [{ system: "http://hl7.org/fhir/sid/icd-10-cm", code: r.code, display: r.description ?? undefined }] } : { text: r.description ?? r.code }, subject: this.refs.reference("Patient", r.patientId), encounter: this.refs.reference("Encounter", r.encounterId), onsetDateTime: r.onsetDate?.toISOString(), abatementDateTime: r.resolvedDate?.toISOString(), recordedDate: r.createdAt.toISOString() }; }
  private async mapRequest(f: string, r: any) { await this.safeCoreRefs(f, r.order); return { resourceType: "ServiceRequest", id: r.id, status: this.requestStatus(r.status), intent: "order", priority: ({ ROUTINE: "routine", URGENT: "urgent", STAT: "stat" } as any)[r.order.priority], code: r.catalogItemId ? { coding: [{ system: "urn:medora:catalog", code: r.catalogItemId }], text: r.manualLabel ?? undefined } : { text: r.manualLabel ?? r.manualSecondaryText ?? "Diagnostic service" }, subject: this.refs.reference("Patient", r.order.patientId), encounter: this.refs.reference("Encounter", r.order.encounterId), authoredOn: r.order.createdAt.toISOString(), occurrenceDateTime: (r.intendedAdministrationAt ?? r.effectiveCollectedAt ?? r.effectivePerformedAt)?.toISOString() }; }
  ...
  private async mapCarePlan(f: string, r: any) { await this.safeCoreRefs(f, r); const practitioner = await this.prisma.userRole.findFirst({ where: { facilityId: f, userId: r.activatedByUserId, isActive: true }, select: { userId: true } }); return { resourceType: "CarePlan", id: r.id, status: ({ DRAFT:"draft", ACTIVE:"active", ON_HOLD:"on-hold", UNDER_REVIEW:"active", COMPLETED:"completed", DISCONTINUED:"revoked" } as any)[r.status], intent: "plan", title: r.title, subject: this.refs.reference("Patient", r.patientId), encounter: this.refs.reference("Encounter", r.encounterId), period: { start: r.activatedAt.toISOString(), end: (r.completedAt ?? r.discontinuedAt)?.toISOString() }, created: r.createdAt.toISOString(), author: practitioner ? this.refs.reference("Practitioner", practitioner.userId) : undefined, activity: r.components.map((c:any) => ({ detail: { status: ({ NOT_STARTED:"not-started", IN_PROGRESS:"in-progress", MET:"completed", PARTIALLY_MET:"in-progress", NOT_MET:"stopped", DISCONTINUED:"stopped" } as any)[c.status], description: c.text } })) }; }
  ...
  private requestStatus(v:string) { return ({ DRAFT:"draft", PENDING:"active", PLACED:"active", SIGNED:"active", ACKNOWLEDGED:"active", IN_PROGRESS:"active", RESULTED:"completed", VERIFIED:"completed", COMPLETED:"completed", CANCELLED:"revoked" } as any)[v] ?? "unknown"; }
  private reportStatus(r:any) { if(r.orderItem.status==="CANCELLED") return "cancelled"; return r.verifiedAt ? "final" : "preliminary"; }
