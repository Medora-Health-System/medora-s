  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<unknown> {
    ...
    return next.handle().pipe(mergeMap(async (body) => {
      if (request.fhirContext?.actorType === "machine" && request.user?.principalType === "fhir-client" && request.fhirCapability) {
        await this.identities.auditAccess(request.user as FhirMachinePrincipal, request.fhirCapability.resourceType, request.fhirCapability.interaction, request.requestId);
      }
      return body;
    }));
  }
