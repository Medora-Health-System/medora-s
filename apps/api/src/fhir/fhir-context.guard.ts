...
export class FhirContextGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext) {
    ...
    if (req.user?.principalType === "fhir-client") {
      ...
      req.fhirContext = {
        actorType: "machine", actorId: clientId, facilityId,
        scopes: Array.isArray(req.user.scopes) ? req.user.scopes : [],
        jurisdiction: facility.country, profiles: effective.map((p) => p.packageId), correlationId: String(req.requestId ?? ""),
        integrationId: String(req.user.integrationId ?? ""), credentialId: String(req.user.credentialId ?? ""), keyId: String(req.user.keyId ?? ""),
