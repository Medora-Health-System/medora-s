...
export class OrdersService {
  ...
  private async assertPsychiatryMedicationOrderAllowed(
    facilityId: string,
    data: OrderCreateDto,
    userId?: string
  ): Promise<void> {
    ...
    for (const row of rows) {
      if (!isActivePsychiatryProviderOrderingMedication(row.code)) continue;
      const validation = validatePsychiatryProviderOrderPlacement({ catalogCode: row.code });
      if (!validation.allowed) {
        logInfo("psychiatry_medication_order_blocked", {
          facilityId,
          userId: userId ?? null,
          catalogMedicationId: row.id,
          catalogCode: row.code,
  ...
  private async assertGastroenterologyMedicationOrderAllowed(
    facilityId: string,
    data: OrderCreateDto,
    userId?: string
  ): Promise<void> {
    ...
    for (const row of rows) {
      if (!isActiveGastroenterologyProviderOrderingMedication(row.code)) continue;
      const validation = validateGastroenterologyProviderOrderPlacement({ catalogCode: row.code });
      if (!validation.allowed) {
        logInfo("gastroenterology_medication_order_blocked", {
          facilityId,
          userId: userId ?? null,
          catalogMedicationId: row.id,
          catalogCode: row.code,
  ...
  private async assertPediatricsMedicationOrderAllowed(
    facilityId: string,
    data: OrderCreateDto,
    userId?: string
  ): Promise<void> {
    ...
    for (const row of rows) {
      if (!isActivePediatricsProviderOrderingMedication(row.code)) continue;
      const validation = validatePediatricsProviderOrderPlacement({ catalogCode: row.code });
      if (!validation.allowed) {
        logInfo("pediatrics_medication_order_blocked", {
          facilityId,
          userId: userId ?? null,
          catalogMedicationId: row.id,
          catalogCode: row.code,
  ...
  private async assertSurgeryPerioperativeMedicationOrderAllowed(
    facilityId: string,
    data: OrderCreateDto,
    userId?: string
  ): Promise<void> {
    ...
    for (const row of rows) {
      if (!isActiveSurgeryPerioperativeProviderOrderingMedication(row.code)) continue;
      const validation = validateSurgeryPerioperativeProviderOrderPlacement({ catalogCode: row.code });
      if (!validation.allowed) {
        logInfo("surgery_perioperative_medication_order_blocked", {
          facilityId,
          userId: userId ?? null,
          catalogMedicationId: row.id,
          catalogCode: row.code,
  ...
  async create(
    ...
    await this.assertPsychiatryMedicationOrderAllowed(facilityId, data, userId);
    await this.assertGastroenterologyMedicationOrderAllowed(facilityId, data, userId);
    await this.assertPediatricsMedicationOrderAllowed(facilityId, data, userId);
    await this.assertSurgeryPerioperativeMedicationOrderAllowed(facilityId, data, userId);
