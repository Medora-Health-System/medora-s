/**
 * Mirrors `prisma/helpers/seed-haiti-medication-catalog.ts` `deriveMedicationCode`
 * for read-only backfill analysis (kept under `src/` for API build rootDir).
 */
export function deriveMedicationProductCode(row: {
  genericName: string;
  strength: string;
  dosageForm: string;
  route: string;
