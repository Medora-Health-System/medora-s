...
export class AdminIntegrationsService {
  ...
  async create(userId: string, input: any) {
    ...
    await this.event(userId, row.id, INTEGRATION_AUDIT_EVENTS.CREATED, { facilityCount: input.facilityIds.length, permissionCount: input.permissionCodes.length, protocol: input.protocol }); return this.safeResponse(row);
  }
