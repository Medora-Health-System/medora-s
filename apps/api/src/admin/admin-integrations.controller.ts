...
export class AdminIntegrationsController {
  ...
  @Get(":id/clients")
  async clients(@Req() req: any, @Param("id") id: string) {
    await this.service.authorize(req.user.userId);
    return this.machines.listClients(id);
  }

  @Post(":id/clients")
  async provisionClient(@Req() req: any, @Param("id") id: string, @Body() body: unknown) {
    await this.service.authorize(req.user.userId);
  ...
  @Post(":id/clients/:clientId/credentials/rotate")
  async rotateCredential(@Req() req: any, @Param("id") id: string, @Param("clientId") clientId: string, @Body() body: unknown) {
    await this.service.authorize(req.user.userId);
  ...
  @Post(":id/clients/:clientId/credentials/:keyId/revoke")
  async revokeCredential(@Req() req: any, @Param("id") id: string, @Param("clientId") clientId: string, @Param("keyId") keyId: string) {
    await this.service.authorize(req.user.userId);
    return this.machines.revokeCredential(req.user.userId, id, clientId, keyId);
  }

  @Post(":id/clients/:clientId/revoke")
  async revokeClient(@Req() req: any, @Param("id") id: string, @Param("clientId") clientId: string) {
    await this.service.authorize(req.user.userId);
