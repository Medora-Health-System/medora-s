...
import { parseOptionalIcd10ListLocale } from "./icd10-search-locale";
...
export class DiagnosesService {
  ...
  async findByPatient(
    ...
    const locale = parseOptionalIcd10ListLocale(query.locale);
    const enriched = await this.attachDiagnosisCreatorDisplay(facilityId, items);
    const presented = await attachIcd10DiagnosisPresentation(this.terminology, locale, enriched);
    return { items: presented.map(stripIcd10CatalogFromPresentedRow), total };
  }
