...
import {
  ...
  type ClaimAssemblyPackageType,
  type ClaimAssemblyPreviewStatus,
} from "@medora/shared";
...
@Injectable()
export class ClaimAssemblyPreviewService {
  constructor(private readonly prisma: PrismaService) {}

  async getForEncounter(params: { encounterId: string; facilityId: string }) {
    const summary = await this.buildEncounterClaimAssemblyPreview(params.encounterId, params.facilityId);
    if (!summary) throw new NotFoundException("Encounter not found");
    return summary;
  }

  async getQueue(filters: ClaimAssemblyPreviewQueueFilters) {
    ...
    const [diagGroup, billingEvents, pendingOrderGroup, reassessmentGroup] = await Promise.all([
    ...
    for (const encounter of encounters) {
      const computed = this.computePreviewFromData({
        encounter,
        ...
        billingEvents: eventsByEncounter.get(encounter.id) ?? [],
      ...
      if (filters.status && computed.status !== filters.status) continue;
      if (filters.packageType && computed.packageType !== filters.packageType) continue;
      if (filters.manualReviewOnly && !computed.requiresManualReview) continue;
      if (filters.professionalOnly && !computed.professionalPackage.applies) continue;
      if (filters.facilityOnly && !computed.facilityPackage.applies) continue;

      const { layers: _layers, claimAssembly: _ca, ...queueRow } = computed;
      rows.push(queueRow);
    }
  ...
  private async buildEncounterClaimAssemblyPreview(encounterId: string, facilityId: string) {
    const nowMs = Date.now();
    ...
    const [diagnosisCount, billingEvents, payerResolution, pendingOrders, observationReassessmentCount] =
      await Promise.all([
        ...
        this.prisma.encounterClinicalEvent.count({
          where: {
            facilityId,
            encounterId,
            payloadJson: { path: ["source"], equals: "OBSERVATION_REASSESSMENT_V1" },
    ...
    const computed = this.computePreviewFromData({
      ...
      observationReassessmentEventCount: observationReassessmentCount,
      nowMs,
      hasPayer: payerResolution.ok,
    });

    const { layers, claimAssembly: _ca, ...row } = computed;
    return {
      ...row,
      ...
      chargeReview: layers.chargeReview,
      codingReview: layers.codingReview,
      previewOnly: true as const,
    };
  }

  private computePreviewFromData(args: {
    encounter: {
      id: string;
      type: string;
      status: string;
    ...
    let hasProfessionalLedger = false;
    let hasFacilityLedger = false;
    ...
    const documentationCompleteness = deriveDocumentationCompletenessFlags({
      ...
      hasPrimaryDiagnosis,
      hasProviderAttribution,
      observationReassessmentEventCount: args.observationReassessmentEventCount,
    });
    ...
    return {
      ...
      billingClassification: classification,
      status: claimAssembly.status,
      packageType: claimAssembly.packageType,
      professionalReady: claimAssembly.professionalPackage.ready,
      facilityReady: claimAssembly.facilityPackage.ready,
      professionalPackage: claimAssembly.professionalPackage,
      facilityPackage: claimAssembly.facilityPackage,
      reasons: claimAssembly.reasons,
      warnings: claimAssembly.warnings,
      requiresManualReview: claimAssembly.requiresManualReview,
      missingItemsCount: claimAssembly.reasons.length,
      nextOperationalAction: claimAssembly.status,
      claimAssembly,
      previewOnly: true as const,
      layers: {
        exportReadiness,
