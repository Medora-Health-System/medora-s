...
import {
  ...
  type CodingIntegrityDomain,
  type CodingIntegrityStatus,
} from "@medora/shared";
...
@Injectable()
export class CodingIntegrityReviewService {
  constructor(private readonly prisma: PrismaService) {}

  async getForEncounter(params: { encounterId: string; facilityId: string }) {
    const summary = await this.buildEncounterCodingReview(params.encounterId, params.facilityId);
    if (!summary) throw new NotFoundException("Encounter not found");
    return summary;
  }

  async getQueue(filters: CodingReviewQueueFilters) {
    ...
    if (filters.observationOnly) {
      where.billingClassification = "OBSERVATION";
    }
    ...
    const [diagGroup, billingEvents, pendingOrderGroup, reassessmentGroup] = await Promise.all([
      ...
      this.prisma.encounterClinicalEvent.groupBy({
        ...
        where: {
          ...
          payloadJson: {
            path: ["source"],
            equals: "OBSERVATION_REASSESSMENT_V1",
          },
        },
        _count: { _all: true },
      }),
    ...
    for (const encounter of encounters) {
      ...
      if (filters.status && computed.codingIntegrityStatus !== filters.status) continue;
      if (filters.domain && !computed.domains.includes(filters.domain)) continue;
      if (filters.providerClarificationOnly && !computed.requiresProviderClarification) continue;
      if (filters.complianceOnly && !computed.requiresComplianceReview) continue;

      const { layers: _layers, documentationCompleteness: _doc, ...queueRow } = computed;
      rows.push(queueRow);
    }
  ...
  private async buildEncounterCodingReview(encounterId: string, facilityId: string) {
    const nowMs = Date.now();
    ...
    const { layers, documentationCompleteness, ...row } = computed;
    return {
      ...row,
  ...
  private computeReviewFromData(args: {
    ...
    return {
      ...
      billingClassification: classification,
      codingIntegrityStatus: codingReview.status,
      domains: codingReview.domains,
      reasons: codingReview.reasons,
      warnings: codingReview.warnings,
      requiresProviderClarification: codingReview.requiresProviderClarification,
      requiresObservationReview: codingReview.requiresObservationReview,
      requiresComplianceReview: codingReview.requiresComplianceReview,
      requiresFacilityReview: codingReview.requiresFacilityReview,
      readyForCodingReview: codingReview.readyForCodingReview,
      hold: codingReview.hold,
      missingItemsCount: codingReview.reasons.length,
      documentationCompleteness,
      previewOnly: true as const,
      layers: {
        exportReadiness,
