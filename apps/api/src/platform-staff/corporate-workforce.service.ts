...
export class CorporateWorkforceService {
  ...
  async upsert(actorUserId:string,targetUserId:string,input:WorkforceInput){
    ...
    if(!staff.isActive)throw new BadRequestException("Target must be active Medora staff");
    ...
    const id=randomUUID();
    const rows=await this.prisma.$queryRawUnsafe<any[]>(`INSERT INTO "MedoraWorkforceProfile" ("id","userId","department","jobTitle","managerUserId","employeeType","employmentStatus","startDate","endDate","createdByUserId","updatedByUserId","createdAt","updatedAt") VALUES ($1,$2,$3::"MedoraCorporateDepartment",$4,$5,$6::"MedoraEmployeeType",$7::"MedoraEmploymentStatus",$8,$9,$10,$10,NOW(),NOW()) ON CONFLICT ("userId") DO UPDATE SET "department"=EXCLUDED."department","jobTitle"=EXCLUDED."jobTitle","managerUserId"=EXCLUDED."managerUserId","employeeType"=EXCLUDED."employeeType","employmentStatus"=EXCLUDED."employmentStatus","startDate"=EXCLUDED."startDate","endDate"=EXCLUDED."endDate","updatedByUserId"=EXCLUDED."updatedByUserId","updatedAt"=NOW() RETURNING *`,id,targetUserId,input.department,input.jobTitle.trim(),input.managerUserId??null,input.employeeType,input.employmentStatus,start,end,actorUserId);
    const row=rows[0];
  ...
  async get(targetUserId:string){
    const staff=await this.prisma.medoraStaffProfile.findUnique({where:{userId:targetUserId},select:{id:true}});
    if(!staff)throw new NotFoundException("Medora staff profile not found");
    const rows=await this.prisma.$queryRawUnsafe<any[]>(`SELECT w.*, u."firstName" AS "managerFirstName", u."lastName" AS "managerLastName" FROM "MedoraWorkforceProfile" w LEFT JOIN "User" u ON u.id=w."managerUserId" WHERE w."userId"=$1`,targetUserId);
    return rows[0]??null;
  }
}
