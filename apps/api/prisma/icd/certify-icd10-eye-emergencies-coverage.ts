...
async function main() {
  ...
  const scoped = selectEyeEmergenciesScopedCodes(validation.parse.rows, { billableOnly: true });
  ...
  try {
    ...
    writeFileSync(join(dir, "fy2026-eye-emergencies-coverage-summary.json"), summary);
    writeFileSync(join(dir, release, "fy2026-eye-emergencies-coverage-summary.json"), summary);
    console.log(summary);
