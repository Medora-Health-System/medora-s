...
async function main() {
  ...
  const scoped = selectEntEmergenciesScopedCodes(validation.parse.rows, { billableOnly: true });
  ...
  try {
    ...
    writeFileSync(join(dir, "fy2026-ent-emergencies-coverage-summary.json"), summary);
    writeFileSync(join(dir, release, "fy2026-ent-emergencies-coverage-summary.json"), summary);
    console.log(summary);
