...
async function main() {
  ...
  const scoped = selectDermatologyScopedCodes(validation.parse.rows, { billableOnly: true });
  ...
  try {
    ...
    writeFileSync(join(dir, "fy2026-dermatology-coverage-summary.json"), summary);
    writeFileSync(join(dir, release, "fy2026-dermatology-coverage-summary.json"), summary);
    console.log(summary);
