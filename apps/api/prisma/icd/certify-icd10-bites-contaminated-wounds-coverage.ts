...
import {
  ...
  selectBiteInfectionComplicationScopedCodes,
...
async function main() {
  ...
  const infection = selectBiteInfectionComplicationScopedCodes(validation.parse.rows, { billableOnly: true });
  ...
  try {
    ...
    const descriptionMismatches = scoped
      .filter(
        (row) =>
          byCode.get(row.code) &&
          byCode.get(row.code)!.shortDescription.trim() !== row.shortDescription.trim(),
      )
      .map((row) => row.code);
    ...
    const report = {
      bitesContaminatedWounds: {
        ...
        infectionScoped: infection.length,
      },
      certification: {
        pass:
          !missingCodes.length &&
    ...
    writeFileSync(join(dir, "fy2026-bites-contaminated-wounds-coverage-summary.json"), summary);
    const releaseDir = join(dir, String(validation.manifest.releaseYear));
    mkdirSync(releaseDir, { recursive: true });
    writeFileSync(join(releaseDir, "fy2026-bites-contaminated-wounds-coverage-summary.json"), summary);
