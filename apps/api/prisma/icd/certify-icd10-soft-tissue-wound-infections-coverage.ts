...
async function main() {
  ...
  const scoped = selectSoftTissueWoundInfectionsScopedCodes(validation.parse.rows, { billableOnly: true });
  ...
  try {
    ...
    writeFileSync(join(dir, "fy2026-soft-tissue-wound-infections-coverage-summary.json"), summary);
    writeFileSync(join(dir, release, "fy2026-soft-tissue-wound-infections-coverage-summary.json"), summary);
    console.log(summary);
