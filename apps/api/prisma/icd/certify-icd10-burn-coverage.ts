/**
 * Certify burn, frostbite, electrical injury, and sunburn ICD coverage against an official release.
 *
 * pnpm --filter @medora/api icd:coverage:burns -- --file=/path/to/official-release.zip --release=2026 --write-reports
 */
...
async function main() {
  ...
  const official = selectBurnScopedCodes(validation.parse.rows, { billableOnly: true });
  ...
  try {
    ...
    const report = {
      ...
      certification: {
        burnPass: missingCodes.length === 0 && invalidSelectableHeaders === 0,
    ...
    writeFileSync(join(summaryDir, "fy2026-burn-coverage-summary.json"), JSON.stringify(report, null, 2));
    if (hasFlag("write-reports")) {
      ...
      writeFileSync(join(outDir, "burn-coverage.json"), JSON.stringify(report.burn, null, 2));
      console.error(`Wrote reports to ${outDir}`);
    }
    process.exit(report.certification.burnPass && report.certification.overallParityPass ? 0 : 2);
  } finally {
