/**
 * Certify blast injury / polytrauma ICD coverage against an official release.
 *
 * pnpm --filter @medora/api icd:coverage:blast-polytrauma -- --file=/path/to/official-release.zip --release=2026 --write-reports
 */
...
async function main() {
  ...
  const official = selectBlastPolytraumaScopedCodes(validation.parse.rows, { billableOnly: true });
  ...
  try {
    ...
    const duplicateCodes = medoraRows
      .map((row) => row.code)
    ...
    const report = {
      ...
      certification: {
        blastPolytraumaPass: missingCodes.length === 0 && invalidSelectableHeaders === 0 && duplicateCodes.length === 0,
    ...
    writeFileSync(join(summaryDir, "fy2026-blast-polytrauma-coverage-summary.json"), summary);
    const releaseSummaryDir = join(summaryDir, String(validation.manifest.releaseYear));
    mkdirSync(releaseSummaryDir, { recursive: true });
    writeFileSync(join(releaseSummaryDir, "fy2026-blast-polytrauma-coverage-summary.json"), summary);
    ...
    if (hasFlag("write-reports")) {
      ...
      writeFileSync(join(outDir, "blast-polytrauma-coverage.json"), JSON.stringify(report.blastPolytrauma, null, 2));
      console.error(`Wrote reports to ${outDir}`);
    }
    process.exit(report.certification.blastPolytraumaPass && report.certification.overallParityPass ? 0 : 2);
  } finally {
