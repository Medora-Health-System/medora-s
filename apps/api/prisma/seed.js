...
const patient_sex_map_1 = require("../src/utils/patient-sex-map");
...
async function main() {
    const roles = await Promise.all([
        ...
        { code: client_1.RoleCode.RN, name: "Registered Nurse" },
        { code: client_1.RoleCode.FRONT_DESK, name: "Front Desk" },
        { code: client_1.RoleCode.LAB, name: "Lab" },
        { code: client_1.RoleCode.RADIOLOGY, name: "Radiology" },
        { code: client_1.RoleCode.PHARMACY, name: "Pharmacy" },
        { code: client_1.RoleCode.BILLING, name: "Billing" }
    ].map((r) => prisma.role.upsert({
        where: { code: r.code },
        update: { name: r.name },
        create: { code: r.code, name: r.name }
    })));
    const adminRole = roles.find((r) => r.code === client_1.RoleCode.ADMIN);
    if (!adminRole)
    ...
    const deptDefs = [
        { code: client_1.DepartmentCode.PRIMARY_CARE, name: "Primary Care" },
        { code: client_1.DepartmentCode.LAB, name: "Laboratory" },
        { code: client_1.DepartmentCode.RAD, name: "Radiology" },
        { code: client_1.DepartmentCode.PHARM, name: "Pharmacy" },
        { code: client_1.DepartmentCode.INPATIENT, name: "Inpatient" }
    ];
    await Promise.all([facilityDR, facilityHT].flatMap((facility) => deptDefs.map((d) => prisma.department.upsert({
        where: { facilityId_code: { facilityId: facility.id, code: d.code } },
        update: { name: d.name },
        create: { facilityId: facility.id, code: d.code, name: d.name }
    }))));
    await Promise.all(haiti_geo_departments_1.HAITI_GEO_DEPARTMENTS.map((d) => prisma.geoDepartment.upsert({
        where: { code: d.code },
        update: { name: d.name, sortOrder: d.sortOrder },
        create: { code: d.code, name: d.name, sortOrder: d.sortOrder },
    })));
    for (const sc of haiti_seed_communes_1.HAITI_SEED_COMMUNES) {
    ...
    await Promise.all([facilityDR.id, facilityHT.id].map((facilityId) => prisma.userRole.upsert({
        where: {
    ...
    const providerRole = roles.find((r) => r.code === client_1.RoleCode.PROVIDER);
    const rnRole = roles.find((r) => r.code === client_1.RoleCode.RN);
    const pharmacyRole = roles.find((r) => r.code === client_1.RoleCode.PHARMACY);
    const frontDeskRole = roles.find((r) => r.code === client_1.RoleCode.FRONT_DESK);
    const htPrimaryDept = await prisma.department.findFirst({
        where: { facilityId: facilityHT.id, code: client_1.DepartmentCode.PRIMARY_CARE },
    });
    const htPharmDept = await prisma.department.findFirst({
        where: { facilityId: facilityHT.id, code: client_1.DepartmentCode.PHARM },
    });
    const drPrimaryDept = await prisma.department.findFirst({
        where: { facilityId: facilityDR.id, code: client_1.DepartmentCode.PRIMARY_CARE },
    });
    const drPharmDept = await prisma.department.findFirst({
        where: { facilityId: facilityDR.id, code: client_1.DepartmentCode.PHARM },
    });
    const demoUsers = [
    ...
    for (const u of demoUsers) {
        ...
        const drDeptId = u.roleId === pharmacyRole.id ? drPharmDept?.id ?? null : drPrimaryDept?.id ?? null;
        await prisma.userRole.upsert({
            where: {
    ...
    const labRole = roles.find((r) => r.code === client_1.RoleCode.LAB);
    const radRole = roles.find((r) => r.code === client_1.RoleCode.RADIOLOGY);
    const htLabDept = await prisma.department.findFirst({
        where: { facilityId: facilityHT.id, code: client_1.DepartmentCode.LAB },
    });
    const htRadDept = await prisma.department.findFirst({
        where: { facilityId: facilityHT.id, code: client_1.DepartmentCode.RAD },
    });
    const drLabDept = await prisma.department.findFirst({
        where: { facilityId: facilityDR.id, code: client_1.DepartmentCode.LAB },
    });
    const drRadDept = await prisma.department.findFirst({
        where: { facilityId: facilityDR.id, code: client_1.DepartmentCode.RAD },
    });
    ...
    const billingRole = roles.find((r) => r.code === client_1.RoleCode.BILLING);
    ...
    const medCatalogIds = await (0, seed_haiti_medication_catalog_1.seedHaitiMedicationCatalog)(prisma, haiti_medications_1.HAITI_MEDICATION_CATALOG);
    ...
    const patientDefs = [
        { mrn: "1001", firstName: "Jean", lastName: "Pierre", dob: "1980-05-12", sex: client_1.SexAtBirth.M, phone: "+509 3701 0001", city: "Port-au-Prince" },
        { mrn: "1002", firstName: "Marie", lastName: "Joseph", dob: "1975-08-22", sex: client_1.SexAtBirth.F, phone: "+509 3701 0002", city: "Cap-Haïtien" },
        { mrn: "1003", firstName: "Claude", lastName: "Saint-Louis", dob: "1992-01-08", sex: client_1.SexAtBirth.M, phone: "+509 3701 0003", city: "Gonaïves" },
        { mrn: "1004", firstName: "Yolande", lastName: "Dupont", dob: "1988-11-30", sex: client_1.SexAtBirth.F, phone: "+509 3701 0004", city: "Saint-Marc" },
        { mrn: "1005", firstName: "Michel", lastName: "François", dob: "1965-03-15", sex: client_1.SexAtBirth.M, phone: "+509 3701 0005", city: "Les Cayes" },
        { mrn: "1006", firstName: "Suzanne", lastName: "Jean-Baptiste", dob: "1995-07-04", sex: client_1.SexAtBirth.F, phone: "+509 3701 0006", city: "Port-au-Prince" },
        { mrn: "1007", firstName: "André", lastName: "Toussaint", dob: "1970-09-19", sex: client_1.SexAtBirth.M, phone: "+509 3701 0007", city: "Jacmel" },
        { mrn: "1008", firstName: "Carmen", lastName: "Larose", dob: "1982-12-01", sex: client_1.SexAtBirth.F, phone: "+509 3701 0008", city: "Cap-Haïtien" },
        { mrn: "1009", firstName: "Ronald", lastName: "Marcelin", dob: "2000-02-14", sex: client_1.SexAtBirth.M, phone: "+509 3701 0009", city: "Port-au-Prince" },
        { mrn: "1010", firstName: "Joceline", lastName: "Auguste", dob: "1958-06-25", sex: client_1.SexAtBirth.F, phone: "+509 3701 0010", city: "Saint-Marc" },
        {
            mrn: "1011",
            firstName: "Patient",
    ...
    for (const p of patientDefs) {
        ...
        const sexVal = sexAtBirthVal != null ? (0, patient_sex_map_1.sexAtBirthToPatientSex)(sexAtBirthVal) : client_1.PatientSex.UNKNOWN;
    ...
    if (existingSeedEncounters === 0 && providerUser && pharmacyUser && rnUser) {
        ...
        const favoriteCatalogIds = haiti_medications_1.HAITI_DEFAULT_FAVORITE_CODES.map((c) => medCatalogIds[c]).filter(Boolean);
        ...
        for (const e of encounterDefs) {
            ...
            const enc = await prisma.encounter.create({
                data: {
                    ...
                    type: client_1.EncounterType.OUTPATIENT,
                    status: e.daysAgo <= 1 ? client_1.EncounterStatus.OPEN : client_1.EncounterStatus.CLOSED,
        ...
        const diagnosisDefs = [
            { encounterIdx: 0, code: "I10", description: "Essential hypertension", onsetDate: "2023-01-15", status: client_1.DiagnosisStatus.ACTIVE },
            { encounterIdx: 1, code: "J06.9", description: "Acute upper respiratory infection", onsetDate: "2025-03-10", status: client_1.DiagnosisStatus.ACTIVE },
            { encounterIdx: 2, code: "R10.9", description: "Unspecified abdominal pain", onsetDate: "2025-03-15", status: client_1.DiagnosisStatus.ACTIVE },
            { encounterIdx: 4, code: "E11.9", description: "Type 2 diabetes mellitus", onsetDate: "2020-06-01", status: client_1.DiagnosisStatus.ACTIVE },
            { encounterIdx: 5, code: "L20.9", description: "Atopic dermatitis", onsetDate: "2025-03-12", status: client_1.DiagnosisStatus.ACTIVE },
            { encounterIdx: 6, code: "A09", description: "Infectious gastroenteritis", onsetDate: "2025-03-17", status: client_1.DiagnosisStatus.ACTIVE },
            { encounterIdx: 7, code: "D64.9", description: "Anemia, unspecified", onsetDate: "2024-11-01", status: client_1.DiagnosisStatus.ACTIVE },
            { encounterIdx: 9, code: "M17.9", description: "Osteoarthritis of knee", onsetDate: "2022-05-01", status: client_1.DiagnosisStatus.ACTIVE },
            { encounterIdx: 1, code: "J06.9", description: "URI (resolved)", onsetDate: "2025-02-01", status: client_1.DiagnosisStatus.RESOLVED },
            { encounterIdx: 12, code: "A01.04", description: "Typhoid fever", onsetDate: "2025-03-05", status: client_1.DiagnosisStatus.ACTIVE },
        ];
        for (const d of diagnosisDefs) {
            await prisma.diagnosis.create({
                data: {
                    ...
                    resolvedDate: d.status === client_1.DiagnosisStatus.RESOLVED ? new Date() : null,
                },
            });
        }
        const dispenseDefs = [
        ...
        for (const disp of dispenseDefs) {
            ...
            await prisma.inventoryTransaction.create({
                data: {
                    inventoryItemId: invId,
                    facilityId: facilityHT.id,
                    type: client_1.InventoryTransactionType.DISPENSE,
                    quantity: -disp.qty,
        ...
        const caseDefs = [
            { diseaseCode: "A00", diseaseName: "Cholera", status: client_1.DiseaseCaseStatus.CONFIRMED, patientIdx: 2, commune: "Saint-Marc", department: "Artibonite" },
            { diseaseCode: "A01.04", diseaseName: "Typhoid fever", status: client_1.DiseaseCaseStatus.CONFIRMED, patientIdx: 2, commune: "Saint-Marc", department: "Artibonite" },
            { diseaseCode: "B50.9", diseaseName: "Plasmodium falciparum malaria", status: client_1.DiseaseCaseStatus.CONFIRMED, patientIdx: 4, commune: "Les Cayes", department: "Sud" },
            { diseaseCode: "A90", diseaseName: "Dengue fever", status: client_1.DiseaseCaseStatus.SUSPECTED, patientIdx: 5, commune: "Port-au-Prince", department: "Ouest" },
            { diseaseCode: "A15.0", diseaseName: "Tuberculosis of lung", status: client_1.DiseaseCaseStatus.CONFIRMED, patientIdx: 7, commune: "Cap-Haïtien", department: "Nord" },
            { diseaseCode: "A00", diseaseName: "Cholera", status: client_1.DiseaseCaseStatus.RULED_OUT, patientIdx: 6, commune: "Jacmel", department: "Sud-Est" },
            { diseaseCode: "B50.9", diseaseName: "Malaria", status: client_1.DiseaseCaseStatus.SUSPECTED, patientIdx: 1, commune: "Cap-Haïtien", department: "Nord" },
        ];
    ...
    await prisma.auditLog.createMany({
        data: [
            {
                action: client_1.AuditAction.SEED,
                entityType: "SYSTEM",
                entityId: "seed",
