/**
 * Wave 3 import platform — source-aware staging + CatalogMedication-first APPLY.
...
import {
  MK_EXPANSION_WAVE3_IMPORTER_VERSION,
...
const DEFAULT_DATA = resolve(__dirname, "data/medora-curated-wave3-candidates.json");
const OUT_DIR = resolve(__dirname, "../audit-summaries");

export type Wave3Baseline = {
  generatedAt: string;
  catalogTotal: number;
...
export function writeWave3Artifact(filename: string, payload: unknown): string {
...
export function loadWave3Candidates(filePath = DEFAULT_DATA): {
  candidates: MkExpansionWave3Candidate[];
  checksumSha256: string;
  fileName: string;
} {
  if (!existsSync(filePath)) {
    throw new Error(`Wave 3 source file missing: ${filePath}`);
  }
  const buf = readFileSync(filePath);
  const checksumSha256 = createHash("sha256").update(buf).digest("hex");
  const raw = JSON.parse(buf.toString("utf8")) as MkExpansionWave3Candidate[];
  if (!Array.isArray(raw) || raw.length === 0) {
    throw new Error("Wave 3 candidates empty or invalid");
  }
  return {
    candidates: raw,
    checksumSha256,
...
export async function collectWave3Baseline(prisma: PrismaClient): Promise<Wave3Baseline> {
  const [
    catalogTotal,
    catalogActive,
    aliases,
...
async function ensureConcept(
  prisma: PrismaClient,
  candidate: MkExpansionWave3Candidate,
  dryRun: boolean
): Promise<{ id: string; created: boolean }> {
  const code = mkExpansionWave3ConceptCode(candidate.conceptKey);
  const existing = await prisma.medicationConcept.findUnique({ where: { code } });
  if (existing) return { id: existing.id, created: false };

  const key = normalizeMkExpansionWave3ConceptKey(candidate.conceptKey);
  const slug = key.toUpperCase().replace(/\s+/g, "_").replace(/[^A-Z0-9_]/g, "");
  for (const prefix of ["EM_W2C_", "ENT_W4_"]) {
...
async function applyVariant(input: {
  prisma: PrismaClient;
  candidate: MkExpansionWave3Candidate;
  variant: MkExpansionWave3Candidate["variants"][number];
  catalogCode: string;
  conceptId: string;
  dryRun: boolean;
  jobId: string;
  ...
  if (await prisma.medicationProduct.findUnique({ where: { code: productCode } })) {
    productCode = `${catalogCode}_W3`.slice(0, 120);
  }
  ...
  const product = await prisma.medicationProduct.create({
    data: {
      ...
      baselineSourceRowId: `${jobId}:${candidate.conceptKey}`.slice(0, 128),
      dualLayerLinkageStatus: "LINKED",
      dualLayerLinkageMethod: "WAVE3_IMPORT",
      governanceNotes: `Wave3 import; domain=${candidate.domain}; source=MEDORA_CURATED; no RxNorm/NDC fabricated`,
    },
  });
  ...
  if (!(await prisma.medicationSafetyProfile.findUnique({ where: { conceptId } }))) {
...
export type Wave3ImportResult = {
  mode: MkExpansionWave3PipelineMode;
  jobId: string;
  ...
  dryRun: boolean;
  baselineBefore: Wave3Baseline | null;
  baselineAfter: Wave3Baseline | null;
  rowsReceived: number;
  rowsValid: number;
  rowsInvalid: number;
  stagedRows: number;
...
export async function runWave3Import(
  prisma: PrismaClient,
  mode: MkExpansionWave3PipelineMode,
  options: { sourceKey?: string; filePath?: string } = {}
): Promise<Wave3ImportResult> {
  assertMkExpansionWave3SafetyDefaults();
  ...
  const dryRun = !mutate;
  const jobId = `W3_${mode}_${Date.now()}`;

  const baselineBefore = await collectWave3Baseline(prisma);
  const { candidates, checksumSha256, fileName } = loadWave3Candidates(
  ...
  if (mode === "AUDIT" || mode === "VALIDATE") {
    const invalid = candidates.filter(
      (c) =>
        !normalizeMkExpansionWave3ConceptKey(c.conceptKey) ||
        !c.variants?.length ||
        normalizeMkExpansionWave3ConceptKey(c.conceptKey) !==
          normalizeMkExpansionWave3ConceptKey(c.genericName)
    );
    const result: Wave3ImportResult = {
      mode,
      jobId,
      importerVersion: MK_EXPANSION_WAVE3_IMPORTER_VERSION,
      sourceKey,
  ...
  for (const candidate of candidates) {
    ...
    const key = normalizeMkExpansionWave3ConceptKey(candidate.conceptKey);
    const genericKey = normalizeMkExpansionWave3ConceptKey(candidate.genericName);
    ...
    const classified = classifyMkExpansionWave3Candidate({
      conceptKey: key,
  ...
  if (mode === "VERIFY" || mode === "APPLY" || mode === "REPORT" || mode === "RECONCILE") {
    result.baselineAfter = await collectWave3Baseline(prisma);
  }

  return result;
}

/** Merge duplicate EM_W3C_* shells that share genericName. */
export async function reconcileWave3Concepts(prisma: PrismaClient): Promise<{
