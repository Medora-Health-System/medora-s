...
export class EnterpriseWave4EdHospitalFormularySeedError extends Error {
  constructor(
    ...
    super(message);
    this.name = "EnterpriseWave4EdHospitalFormularySeedError";
  }
}

export type SeedEnterpriseWave4EdHospitalFormularyOptions = {
  dryRun?: boolean;
};

export type SeedEnterpriseWave4EdHospitalFormularyResult = {
  dryRun: boolean;
  ...
  /** M1.7C — ENRICH alreadyLinked rows that received Wave 4 marker on governanceNotes. */
  wave4GovernanceNotesUpdated: number;
...
/**
 * M1.7C — Enterprise Wave 4 ED/Hospital formulary seed (strict localization + billing + governance).
 * Catalog and products remain inactive; orderSearchEnabled stays false until activation.
 */
export async function seedEnterpriseWave4EdHospitalFormulary(
  prisma: PrismaClient,
  options: SeedEnterpriseWave4EdHospitalFormularyOptions = {}
): Promise<SeedEnterpriseWave4EdHospitalFormularyResult> {
  const dryRun = options.dryRun === true;
  const modules = await loadEnterpriseWave4EdHospitalFormularySeedModules();
  modules.assertEnterpriseWave4EdHospitalFormularyManifest();

  const manifest = modules.ENTERPRISE_WAVE4_ED_HOSPITAL_FORMULARY_MANIFEST ?? [];
  const billingByCode = modules.ENTERPRISE_WAVE4_ED_HOSPITAL_BILLING_BY_CODE ?? {};

  const result: Omit<SeedEnterpriseWave4EdHospitalFormularyResult, "readinessReport"> & {
    readinessReport?: SeedEnterpriseWave4EdHospitalFormularyResult["readinessReport"];
  } = {
    dryRun,
  ...
  for (const rawEntry of manifest) {
    ...
    const upsertBody = {
      ...
      administrationType: resolvedAdministrationType,
    ...
    if (!product && !dryRun) {
      ...
      product = await prisma.medicationProduct.create({
        data: {
          ...
          governanceNotes: mergeEnterpriseWave4EdHospitalGovernanceNotes(null),
        },
        include: {
          concept: true,
          packages: { include: { billingProfiles: true } },
    ...
    const billingReadiness = modules.validateWave4MedicationBillingReadiness(
      entry.catalogCode,
      {
        ...billingSnapshot,
  ...
  if (result.conflicts.length > 0) {
    throw new EnterpriseWave4EdHospitalFormularySeedError(
      `[enterprise-wave4-ed-hospital-formulary] ${result.conflicts.length} conflict(s)`,
      result.conflicts
    );
  }

  const readinessReport = modules.summarizeEnterpriseWave4EdHospitalReadiness(
    perMedication,
    {
      manifestEntries: manifest.length,
    ...
    modules.countWave4GovernanceMarkers()
  );
