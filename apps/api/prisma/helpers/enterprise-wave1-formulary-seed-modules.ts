...
async function importSharedDistModule<T>(fileBaseName: string): Promise<T> {
  ...
  if (!existsSync(modulePath)) {
    throw new Error(
      `[enterprise-wave1-seed] missing ${modulePath} — run pnpm --filter @medora/shared build first`
    );
  }
...
export async function loadEnterpriseWave1FormularySeedModules() {
  if (isJestRuntime()) {
    const formulary = requireSharedSrcModule<
      typeof import("../../../../packages/shared/src/medication/enterpriseWave1FormularyManifest")
    >("enterpriseWave1FormularyManifest");
    const billing = requireSharedSrcModule<
      typeof import("../../../../packages/shared/src/medication/enterpriseWave1BillingManifest")
    >("enterpriseWave1BillingManifest");
    const validation = requireSharedSrcModule<
      typeof import("../../../../packages/shared/src/medication/enterpriseWave1FormularyValidation")
    >("enterpriseWave1FormularyValidation");
    ...
    const search = requireSharedSrcModule<
      typeof import("../../../../packages/shared/src/medication/enterpriseWave1SearchValidation")
    >("enterpriseWave1SearchValidation");
    return {
      ...formulary,
      ...billing,
      ...validation,
