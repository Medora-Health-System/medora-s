...
export class EnterpriseWave1FormularySeedError extends Error {
  constructor(
    ...
    super(message);
    this.name = "EnterpriseWave1FormularySeedError";
  }
}

export type SeedEnterpriseWave1FormularyOptions = {
  dryRun?: boolean;
};

export type SeedEnterpriseWave1FormularyResult = {
  dryRun: boolean;
...
/**
 * M1.6B — Enterprise Wave 1 formulary seed (catalog + canonical chain + billing + governance).
 * Products remain inactive until explicit activation with billing gate.
 */
export async function seedEnterpriseWave1Formulary(
  prisma: PrismaClient,
  options: SeedEnterpriseWave1FormularyOptions = {}
): Promise<SeedEnterpriseWave1FormularyResult> {
  const dryRun = options.dryRun === true;
  const modules = await loadEnterpriseWave1FormularySeedModules();
  modules.assertEnterpriseWave1FormularyManifest();

  const manifest = modules.ENTERPRISE_WAVE1_FORMULARY_MANIFEST;
  const billingByCode = modules.ENTERPRISE_WAVE1_BILLING_BY_CODE;

  const result: Omit<SeedEnterpriseWave1FormularyResult, "readinessReport"> & {
    readinessReport?: SeedEnterpriseWave1FormularyResult["readinessReport"];
  } = {
    dryRun,
  ...
  for (const entry of manifest) {
    ...
    const conceptCode = modules.wave1ConceptCodeForGeneric(entry.genericName);
    const productCode = entry.catalogCode;
    const packageCode = modules.wave1PackageCodeForProduct(productCode);
    ...
    const searchCheck = modules.validateWave1EntrySearchReady(entry, catalogRow
      ? {
    ...
    const billingReadiness = modules.validateWave1MedicationBillingReadiness(
      entry.catalogCode,
      billingSnapshot
    );
    ...
    perMedication.push({
      catalogCode: entry.catalogCode,
      pass: billingReadiness.billingPass && searchCheck.pass,
  ...
  if (result.conflicts.length > 0) {
    throw new EnterpriseWave1FormularySeedError(
      `[enterprise-wave1-formulary] ${result.conflicts.length} conflict(s)`,
      result.conflicts
    );
  }

  const readinessReport = modules.summarizeEnterpriseWave1Readiness(perMedication, {
    manifestEntries: manifest.length,
