/**
 * Haiti MVP demo seed: users, patients, encounters, inventory, public-health demo rows.
 * DEV/DEMO ONLY — skipped by MEDORA_SEED_PROFILE=enterprise.
 * Never logs passwords or PHI payloads.
 */
...
export const SEED_DEMO_HAITI_MARKER = "SEED_DEMO_HAITI";

/** Local demo bootstrap password — never printed to logs. */
const DEMO_PASSWORD = "MedoraAdmin123!";

export async function seedDemoHaiti(params: {
  prisma: PrismaClient;
  roles: Role[];
  adminRole: Role;
  medoraSuperAdminRole: Role;
  facilityDR: Facility;
  facilityHT: Facility;
  medCatalogIds: Record<string, string>;
  vaccineCatalogIds: Record<string, string>;
}) {
  const {
    prisma,
    roles,
    adminRole,
    medoraSuperAdminRole,
    facilityDR,
    facilityHT,
    medCatalogIds,
    vaccineCatalogIds,
  ...
  const demoUsers = [
    {
      email: "provider@medora.local",
      firstName: "Jean",
      lastName: "Baptiste",
      roleId: providerRole.id,
      departmentId: htPrimaryDept?.id ?? null,
    },
    {
      email: "rn@medora.local",
      firstName: "Marie",
      lastName: "Claire",
      roleId: rnRole.id,
      departmentId: htPrimaryDept?.id ?? null,
    ...
    {
      email: "pharmacy@medora.local",
      firstName: "Pierre",
      lastName: "Louis",
      roleId: pharmacyRole.id,
      departmentId: htPharmDept?.id ?? null,
    },
    {
      email: "frontdesk@medora.local",
      firstName: "Sophie",
      lastName: "Martel",
  ...
  const worklistDemoUsers = [
    {
      email: "lab@medora.local",
      firstName: "Luc",
      lastName: "Laborant",
      roleId: labRole.id,
      htDept: htLabDept?.id ?? null,
      drDept: drLabDept?.id ?? null,
    },
    {
  ...
  const existingSeedEncounters = await prisma.encounter.count({
    where: { facilityId: facilityHT.id, notes: SEED_DEMO_HAITI_MARKER },
  });
    ...
    data: [
      {
        ...
        entityType: "SYSTEM",
        entityId: "seed",
        metadata: { note: "Demo Haiti seed completed", profile: "demo" },
      },
      {
        action: AuditAction.SEED,
        entityType: "USER",
        ...
        userId: adminUser.id,
        metadata: { note: "Seeded demo admin user" },
      },
    ],
    skipDuplicates: true,
  });
  ...
  const vaccineAdminCount = await prisma.vaccineAdministration.count({
    where: { facilityId: facilityHT.id },
  });
  const caseReportCount = await prisma.diseaseCaseReport.count({
    where: { facilityId: facilityHT.id },
  });
  ...
  console.log("\n--- Seeded demo users (credentials not logged) ---");
  console.log("  admin@medora.local");
  console.log("  provider@medora.local");
  console.log("  rn@medora.local");
  console.log("  pharmacy@medora.local");
  console.log("  frontdesk@medora.local");
  console.log("  lab@medora.local");
  console.log("  radiology@medora.local");
  console.log("  billing@medora.local");
  console.log("----------------------------------------\n");
}
