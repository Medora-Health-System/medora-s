...
describe("haitiCanonicalMedicationLinkageManifest", () => {
  it("has 247 Haiti formulary entries", () => {
    expect(HAITI_CANONICAL_LINKAGE_MANIFEST.length).toBe(247);
    expect(HAITI_CANONICAL_LINKAGE_MANIFEST_EXPECTED_COUNT).toBe(247);
  });

  it("indexes by catalogMedicationCode without duplicates", () => {
    expect(Object.keys(HAITI_CANONICAL_LINKAGE_BY_CATALOG_CODE).length).toBe(247);
  });
  ...
  it("defaults to MISSING_CANONICAL_TARGET or MANUAL_REVIEW (no bulk LINK_READY)", () => {
    ...
    expect(missing.length + manual.length).toBe(247);
  });
  ...
  it("passes strict validation against formulary source", () => {
    ...
    expect(result.stats.total).toBe(247);
  });
