...
export function validatePayloadForCard(
  ...
  const infusionResult = validateHighAlertInfusionPayloadForCard(cardId, payload);
  if (
    infusionResult.ok ||
  ...
  const strokeNeuroResult = validateStrokeNeuroReassessmentPayloadForCard(cardId, payload);
  if (
    strokeNeuroResult.ok ||
  ...
  const painResult = validatePainDocumentationPayloadForCard(cardId, payload);
  if (
    painResult.ok ||
  ...
  const proceduralResult = validateProceduralSafetyThrombolyticPayloadForCard(cardId, payload);
  if (
    proceduralResult.ok ||
