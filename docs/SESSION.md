...
- **Layout `/app`** : `GET /api/auth/me` exige `res.ok` (sinon redirection vers `/login`) ; **intervalle** toutes les **4 minutes** vers `POST /api/auth/refresh` pour prolonger la session pendant le travail.
...
- **Travail normal** : pas de déconnexion toutes les 2 minutes ; navigation et appels API déclenchent refresh sur 401 si besoin ; le timer limite l’expiration silencieuse du JWT.

## Déploiement

Définir **la même** `JWT_ACCESS_TTL` sur l’API et sur l’app web (ex. `8h`).  
